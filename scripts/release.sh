...
if [ "$VERSION" != "$ROOT_VERSION" ]; then
  echo "Error: release version ($VERSION) != root package.json ($ROOT_VERSION)."
  echo "Root package.json is the single source of truth — bump it first, then release the matching version."
  exit 1
fi
