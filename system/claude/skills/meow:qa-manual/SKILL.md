...
**Explicit:**
- `/meow:qa-manual tasks/plans/260315-auth-flow.md --report` → manual QA report
- `/meow:qa-manual tasks/plans/260315-checkout.md --generate` → Playwright .spec.ts
- `/meow:qa-manual https://app.example.com` → explore and test from URL
- `/meow:qa-manual [spec]` → auto-detect use case from spec content
...
| Action | Skill | Why |
|--------|-------|-----|
| DOM interaction (click, fill, type) | `meow:playwright-cli` | Generates Playwright code as side-effect |
| Navigation + page load | `meow:playwright-cli` | goto + wait, code output |
| Visual verification (screenshot) | `meow:agent-browser` | Annotated snapshots |
| Auth flow (login, session, MFA) | `meow:agent-browser` | Session persistence, auth import |
...
4. On credentials → fill form using `meow:playwright-cli`, continue flow
