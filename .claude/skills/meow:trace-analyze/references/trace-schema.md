...
| `run_id` | string | optional | `MEOWKIT_RUN_ID` env var (set by `meow:harness` step-00) |
...
| `harness_run_start` | `meow:harness` step-00 | `{task: str, density: str, model: str, run_id: str}` |
| `contract_signed` | `meow:sprint-contract` sign action | `{slug: str, sprint: int, generator_sha: str, evaluator_sha: str}` |
| `verdict_written` | `meow:evaluate` step-04 | `{slug: str, overall: str, weighted_score: float, hard_fail: bool, evidence_count: int}` |
...
| `benchmark_result` | `meow:benchmark/scripts/run-canary.sh` (inline at manifest write — see C2 fix 260408) | `{run_id: str, tier: str, model: str, manifest_path: str}` |
...
`meow:trace-analyze` uses this pattern in `step-01-ingest.md`.
...
`meow:trace-analyze/step-01-ingest.md` filters records by `schema_version` and warns on unknown versions.
