...
### Standalone Mode (`/meow:jira analyze PROJ-123`)
...
6. Suggest actionable /meow:jira commands
...
`.claude/skills/.venv/bin/python3 .claude/skills/meow:multimodal/scripts/gemini_analyze.py <path>`
...
/meow:jira add-comment {ISSUE-KEY} <this analysis>
/meow:jira transition {ISSUE-KEY} "In Analysis"
```
...
/meow:jira add-comment {ISSUE-KEY} <this analysis>
/meow:jira transition {ISSUE-KEY} "In Analysis"
/meow:jira link {ISSUE-KEY} relates-to {RELATED-KEY}
```
...
### Suggested Actions
> User must review before posting. No auto-execution.
/meow:jira add-comment {ISSUE-KEY} <media findings summary>
```
