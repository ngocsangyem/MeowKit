...
Per `meow:rubric` schema, each rubric has a `hard_fail_threshold` (`FAIL` or `WARN`). Iterate the per-rubric verdicts:
...
```bash
.claude/skills/meow:evaluate/scripts/validate-verdict.sh "$verdict_file"
```
