...
**Cause:** You invoked `/meow:pack .` or a local path that resolves to the same git root as your working directory.
...
**Fixes:**
- For inbound context: use `/meow:scout` instead.
...
- Pack locally instead: `git clone --depth 1 ...` then `/meow:pack <local-path>`.
