...
- Refactor the target first (`/mk:fix` or `/mk:simplify`), OR
- Narrow the scope, OR
...
- **henshin is planning, not building.** If the user expects code after running this skill, clarify: henshin emits a Transformation Spec; `/mk:cook` builds. Two skills, two concerns.
