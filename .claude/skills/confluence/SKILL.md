...
| `analyze PAGE-ID --with-commands` | Analysis + suggested /mk:jira create commands |
...
- Suggested /mk:jira create commands (opt-in via --with-commands)
- Open questions
...
- Create Jira tickets (human runs `/mk:jira create` manually)
...
- **mk:confluence → mk:planning-engine** — spec report informs tech review context

## After the Report

1. Review the report — approve/reject/modify suggested stories
2. For approved stories: run `/mk:jira create` commands (if --with-commands was used)
3. For created tickets: run `/mk:jira evaluate` then `/mk:jira estimate`
4. For planning: run `/mk:planning-engine review` per ticket
