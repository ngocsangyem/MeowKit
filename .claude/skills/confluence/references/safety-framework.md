...
- File paths for `--upload` MUST be under `$CLAUDE_PROJECT_DIR` or an explicitly allowlisted `/tmp/conf-*` prefix. Reject `..` traversal even though `confluence-as` also validates.
