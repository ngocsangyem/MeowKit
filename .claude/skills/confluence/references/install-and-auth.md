...
`npx mewkit setup` auto-installs from `.claude/skills/confluence/scripts/requirements.txt`:
...
Verify: `ls .claude/skills/.venv/bin/ | grep confluence-as`
...
| 127  | `confluence-as` binary not installed — run `npx mewkit setup` |
...
Run `bash .claude/skills/confluence/scripts/smoke-live.sh` once to determine which mode applies for your installed version.
