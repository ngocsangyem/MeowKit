...
mk:jira accepts STRUCTURED input only: `--project`, `--type`, `--summary`, issue keys (e.g. `PROJ-123`), JQL queries.
...
> "This looks like raw ticket content. Run /mk:intake first to analyze, then I'll execute the suggested actions."
...
```
1. Read(".claude/skills/jira/agents/jira-evaluator.md") → agent_def
...
- **Batch** requires generating a JSON array of issue objects (mk:jira builds this from templates, MCP doesn't handle templates).
...
- **mk:confluence → human → mk:jira** — confluence produces spec report, human reviews, then runs /mk:jira create manually
- **mk:jira → mk:planning-engine** — evaluate/estimate output enriches tech review and planning reports
...
mk:jira covers core Jira operations + ticket intelligence (evaluate, estimate, analyze). Single-ticket operations only in v2.

| Need | Use |
|------|-----|
| Core Jira ops + ticket intelligence | **mk:jira** |
| JSM (queues, SLAs, customers) | Atlassian MCP directly |
| Git/CI integration | `gh` CLI + mk:ship |
