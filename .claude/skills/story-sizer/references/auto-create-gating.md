...
--dry-run-only                    optional test-mode flag. Stops AFTER table render, BEFORE AskUserQuestion.
```
...
for record in records:
    if "[NO_ACS]" in record.flags OR record.sizing.refusal_reason:
        ABORT with: "Story <id> has no sizeable signal: <reason>.
                     Resolve in the paste body and re-run /mk:story-sizer --paste."
```
...
```text
for record in records:
    sanitized = bash .claude/skills/jira/scripts/jql-sanitize.sh "<suggested_summary>"
    /mk:jira-search query "project = <PROJECT> AND summary ~ \"<sanitized>\""
...
if recompute_hash != report_hash:
    ABORT with: "Paste body changed since sizing. Re-paste the original or
                 re-run /mk:story-sizer --paste to regenerate the report."
```
...
After the table, exactly ONE `AskUserQuestion`:
...
2. Render the table.
3. STOP — do NOT call `AskUserQuestion`.
