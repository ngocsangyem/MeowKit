...
Use the `hypothesis-testing.md` template in `meow:sequential-thinking` to track the why-chain. Each "why" becomes a hypothesis row; "evidence" fills the Evidence column; counter-analysis drives the Revision section. Five-Whys-Plus is the discipline; sequential-thinking's template is the output.
