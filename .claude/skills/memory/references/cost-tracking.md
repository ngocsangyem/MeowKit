...
- [`/mk:budget` Command](#meowbudget-command)
...
- **On `/mk:budget` command:** Display cost summary. `/mk:budget` is a subcommand of `mk:memory`, not a standalone skill — invoke it while `mk:memory` is active: `/mk:budget [--report|--alert N]`.
...
When the user invokes `/mk:budget`, read `.meowkit/telemetry/cost-log.json` and display:
...
- [ ] `/mk:budget` output is formatted as a readable table