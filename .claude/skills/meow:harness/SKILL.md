...
# meow:harness — Autonomous Multi-Hour Build Orchestration
...
Skip when:
- The task is a single feature, bug fix, or refactor → use `/meow:cook` instead
...
1. **Adaptive density** — picks scaffolding density via `meow:scale-routing` (or `density-select.sh`); honors `MEOWKIT_HARNESS_MODE` env override
...
```
Step 0: Tier Detection      → call meow:scale-routing; pick density (MINIMAL|FULL|LEAN)
Step 1: Plan                → invoke meow:plan-creator --product-level
Step 2: Contract            → invoke meow:sprint-contract (skipped if LEAN + small)
Step 3: Generate            → spawn developer subagent w/ 4-subphase pattern
Step 4: Evaluate            → invoke meow:evaluate
...
- **Don't run on a non-green-field task.** Use `meow:cook` for single features, bug fixes, refactors. The harness is for "build me an app" scope.
...
- **Resumable.** If the harness is killed mid-run, `/meow:harness --resume {run-id}` picks up at the last completed step.
...
| `../meow:plan-creator/` | Phase 1 — product-level planner |
| `../meow:sprint-contract/` | Phase 4 — contract negotiation |
| `../meow:rubric/` | Phase 2 — rubric library |
| `../meow:evaluate/` | Phase 3 — behavioral evaluator |
| `../meow:scale-routing/` | Tier + density emission |
