...
```bash
/meow:harness "build a kanban app" --teams
```

The harness then uses `meow:team-config` to spin up a generator + evaluator team, hands off the contract via shared task queue, and runs the iteration loop with live message passing.

To force subagent mode (override any default that might switch):

```bash
/meow:harness "..." --no-teams
```
...
When teams mode is active, file ownership is enforced via `meow:team-config`'s ownership matrix. The evaluator owns:
...
- `meow:harness` defaults to **subagents** unless `--teams` flag is set
- `meow:cook` (single-task) always uses subagents — teams are not relevant for single-task pipelines
