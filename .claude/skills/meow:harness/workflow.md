...
0. `step-00-tier-detection.md` — Call `meow:scale-routing` (or `density-select.sh`); pick `MINIMAL|FULL|LEAN`; record decision in run report
1. `step-01-plan.md` — Invoke `meow:plan-creator --product-level` (or `--fast` for MINIMAL); produce product-level spec
2. `step-02-contract.md` — Invoke `meow:sprint-contract propose → review → sign` (skipped on LEAN with small sprint OR MINIMAL)
3. `step-03-generate.md` — Spawn `developer` subagent with the 4-subphase pattern; generator implements per signed contract
4. `step-04-evaluate.md` — Spawn `evaluator` subagent via `meow:evaluate`; produces graded verdict
5. `step-05-iterate-or-ship.md` — Verdict PASS → route to shipper (Phase 5 of meowkit 7-phase model); FAIL → loop back to step-3 with feedback (max `--max-iter` rounds; escalate after)
...
```
/meow:harness "build a kanban app" [--tier auto|full|lean|minimal] [--max-iter 3] [--budget 50]
    ↓
Step 0: Tier Detection
    ├── density-select.sh OR meow:scale-routing
    ├── env override: MEOWKIT_HARNESS_MODE
    ├── --tier flag override
    └── Create tasks/harness-runs/{run_id}/
         ↓
Step 1: Plan
    ├── meow:plan-creator --product-level
...
Step 2: Contract (conditional)
    ...
    ├── Otherwise: meow:sprint-contract propose → review → sign
...
Step 3: Generate
    ...
    └── Output: tasks/handoff/{slug}-sprint-{N}.md
         ↓
Step 4: Evaluate
    ├── Invoke meow:evaluate <handoff> --rubric-preset {detected}
...
- **MINIMAL** (Haiku/trivial): Runs `meow:cook` instead of the full harness. No contract, no iteration loop. Cheapest path.
...
If a run is killed mid-flight, resume via `/meow:harness --resume {run_id}`. The orchestrator reads `run.md` to find the last completed step and continues from there. Run reports are append-only — no step is re-executed.
