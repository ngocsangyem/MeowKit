...
description: |
  ...
  meow:sequential-thinking instead.
allowed-tools:
  - Read
  - Grep
  - Glob
...
- **Anything broken / needing a fix** — use `meow:fix` (top-level orchestrator; invokes scout + investigate + sequential-thinking, adds regression tests, writes to memory).
- **Pure diagnostic reasoning without fix intent** — use `meow:sequential-thinking` directly (evidence-based hypothesis testing).
- **Trivial fixes** — typo, rename, single-file change with obvious cause.
- **Known-options trade-off** — use `meow:party` (multi-agent deliberation) or `meow:brainstorming` (solution alternatives).

The line: problem-solving is for being stuck on _approach_. Sequential-thinking is for being stuck on _cause_. `meow:fix` is for anything broken that needs fixing.
