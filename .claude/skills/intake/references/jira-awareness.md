...
Loaded when the user has the `mk:jira-*` family available (i.e. `jira-as` installed via `.claude/scripts/bin/setup-workflow` and `.meowkit/.env` populated with `MEOW_JIRA_*`). When unavailable, skip this file entirely — mk:intake operates tool-agnostic.
...
- [ ] Evaluate complexity: /mk:jira evaluate [TICKET-ID]
- [ ] Estimate story points: /mk:jira estimate [TICKET-ID]  ← only if no story points set
