...
mk:intake completes analysis
        ↓
mk:jira-* family available? (jira-as installed + .meowkit/.env populated)
...
```
jira-as not installed (or .meowkit/.env not populated). Apply these changes manually in Jira:
- Transition [TICKET-ID] → "In Analysis"
- Link [TICKET-ID] blocks [BLOCKER-ID]
- Assign to [username]
- Set Fix Version to [version]

Or run `.claude/scripts/bin/setup-workflow` then populate .meowkit/.env from .meowkit/.env.example to enable the mk:jira-* family.
```
