...
# meow:intake works with any MCP that provides:
...
meow:intake uses only standard MCP tool calls — it does not hardcode any vendor's API.
...
If no MCP is available, meow:intake automatically falls back to manual paste (Option 4).
