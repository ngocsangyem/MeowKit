# Figma Link Detection — meow:intake Extension
...
Figma URL detected
        ↓
Figma MCP available?
    YES → invoke meow:figma analyze [URL] → extract design context
    NO  → ask user: "Export target frames as PNG, then paste or provide path"
          → invoke meow:multimodal or Claude Read on the exported image
        ↓
