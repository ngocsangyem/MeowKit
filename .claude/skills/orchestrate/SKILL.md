---
name: mk:orchestrate
description: "Coordinates staged or parallel jobs across live-verified coding-agent runtimes (Claude Code, Codex, Cursor, and other installed CLIs) and in-session MeowKit subagents. Routes each job by capability and risk tier, isolates parallel writers in git worktrees, captures redacted output, resumes interrupted runs, and blocks completion until an independent arbiter verifies the results. Use when work should be split across multiple runtimes or subagents, run as staged or parallel jobs, and reviewed before handoff. NOT for a single-agent task or the in-session 7-phase flow (use mk:cook / mk:workflow-orchestrator)."
...
Coordinate headless coding-agent jobs and in-session MeowKit subagents through a
...
This skill composes with existing MeowKit primitives instead of reimplementing
them:

- **Worktrees** — delegate lifecycle to `mk:worktree` (create → merge →
  cleanup). See Worktree Isolation below.
- **In-session subagents** — internal jobs spawn the re-authored MeowKit agent
...
- **Tier bridge** — the capability tiers (C1/C2/C3) and risk tiers (R0–R3) in
  ...
  MeowKit's TRIVIAL/STANDARD/COMPLEX model-tier mapping when routing internal
  agents.

## Safety Authority

Orchestration never relaxes MeowKit's non-negotiable rules:
...
- `tasks/reports/` is the canonical MeowKit reports location — do not emit to
