...
description: >-
  ...
  /meow:sprint-contract, "draft a sprint contract", "negotiate scope for sprint",
...
# meow:sprint-contract — File-Based Sprint Contract Protocol
...
- The task is `/meow:fix simple` (Gate 1 already bypassed; contract bypass too)
...
2. Load rubric preset via `meow:rubric/scripts/load-rubric.sh --preset {preset}`
...
```bash
.claude/skills/meow:sprint-contract/scripts/validate-contract.sh tasks/contracts/{path}.md
```
