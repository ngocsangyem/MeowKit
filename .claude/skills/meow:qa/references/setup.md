...
"Your working tree has uncommitted changes. /meow:qa needs a clean tree so each bug fix gets its own atomic commit."
...
1. **Project-scoped test plans:** Check `.claude/memory/projects/` for recent `*-test-plan-*.md` files for this repo
   ...
   ```
2. **Conversation context:** Check if a prior `/meow:plan-ceo-review` or `/meow:plan-ceo-review` produced test plan output in this conversation
