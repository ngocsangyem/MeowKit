...
```
/meow:chom <github-url|web-url|local-path|description> [feature] [--compare|--copy|--improve|--port] [--lean|--auto]
```
...
Escalation: if the feature has ≥3 architectural layers OR stateful workflows, emit handoff text: "Complex flow detected. Before Phase 4, trace via /meow:sequential-thinking, then return to chom." Do NOT auto-invoke any skill.
...
- **API-surface queries use `/meow:pack --compress`**, not chom. If the question is "what's the public API of library X" rather than "should we replicate it," pack's Tree-sitter signature extraction is the right tool. chom is for replication decisions, not API exploration.
- **3+ layers trigger sequential-thinking handoff**, not auto-invocation. Trace the flow via `/meow:sequential-thinking`, then return to chom.
