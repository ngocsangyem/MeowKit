...
Runs `meow:harness` skill — the autonomous generator⇄evaluator build pipeline for green-field products. Preferred over `/cook` for product-level builds ("build me a kanban app").
...
2. **Plan** — product-level spec via `meow:plan-creator --product-level`
