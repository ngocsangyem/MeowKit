...
1. **Phase 1 — Plan.** Run `mk:plan-creator` with the feature description.
   - Print: `Phase 1: Planning...`
   - Wait for **Gate 1 approval** from the human.
   - If rejected, revise plan and re-request approval.

2. **Phase 2 — Test.**
   - **TDD mode (`--tdd` / `MEOWKIT_TDD=1`):** Run `mk:testing --red-only` to write failing tests.
...
4. **Phase 4 — Review.** Run `mk:review` for 5-dimension structural audit.
...
5. **Phase 5 — Ship.** Run `mk:ship` after Gate 2 approval.
