...
`/mk:status` runs regardless of `MEOWKIT_PM_AUTO` — the env var only gates silent background fires per `post-phase-delegation.md`. User-invoked requests are always honored.
...
- Alternative (forward-looking): `/mk:help`
