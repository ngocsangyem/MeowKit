#!/usr/bin/env bash
# SessionStart hook: validate .meowkit/.env presence + 3 MEOW_JIRA_* vars.
...
ENV_FILE="${CLAUDE_PROJECT_DIR:-$PWD}/.meowkit/.env"
if [ ! -f "$ENV_FILE" ]; then
  echo "[mk:jira] .meowkit/.env missing — see .meowkit/.env.example for setup"
  exit 0
fi
...
for key in MEOW_JIRA_API_TOKEN MEOW_JIRA_EMAIL MEOW_JIRA_SITE_URL; do
  if ! grep -qE "^${key}=" "$ENV_FILE" 2>/dev/null; then
    echo "[mk:jira] $key missing in .meowkit/.env"
    exit 0
  fi
done
echo "[mk:jira] env OK"
