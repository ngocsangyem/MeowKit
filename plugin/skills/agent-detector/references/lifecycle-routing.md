...
| Deep multi-source technical research with a cited report ("research X", "evaluate X vs Y", "how do others solve Z", "find best practices for X") | any | mk:research (delegates to researcher subagent(s), primary-source discipline + retrieval-call cap, one cited report, optional `--html`; NOT single-library docs — see mk:docs-finder; NOT project-only Q&A — see mk:ask-me) |
