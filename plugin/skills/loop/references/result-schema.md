...
framework `tasks/` tree — no skill-directory writes, and no `${CLAUDE_PLUGIN_DATA}` needed
...
Framework dir → exempt from the persistent-state rule (no `${CLAUDE_PLUGIN_DATA}`). A second
