...
const EVENT_FLAG: Record<string, string> = { APPROVE: "--approve", REQUEST_CHANGES: "--request-changes", COMMENT: "--comment" };
...
export async function reviewSubmit(options: SubmitOptions): Promise<SubmitResult> {
	...
	if (!options.reply) return bail("submit requires --reply (the only GitHub write authority); without it the run is review-only");
	...
	if (!parsed.success) return bail(`submit payload failed schema: ${parsed.error.issues.map((i) => i.message).join("; ")}`);
	const hash = payloadHashOf(payload);

	// 2. Confirmation bound to THIS payload.
	if (!options.confirm) return bail(`submit requires --confirm <payload-hash> after the user approves the composed body. Current payload hash: ${hash}`);
	if (options.confirm !== hash) return bail("confirmation does not match the current payload (stale or recomposed) — re-confirm the shown body");

	// 3. Idempotency: never double-post.
	const marker = path.join(sessionDir, "submitted.json");
	if (fs.existsSync(marker)) {
		try { if (JSON.parse(fs.readFileSync(marker, "utf-8")).payloadHash === hash) return emit({ ok: true, posted: false, alreadyPosted: true, payloadHash: hash }); } catch { /* fall through to re-post */ }
	}

	// 4. Re-fetch the PR head SHA; a change since prepare aborts WITHOUT posting.
	const head = exec("gh", ["api", `repos/${payload.owner}/${payload.repo}/pulls/${payload.pr}`, "--jq", ".head.sha"], cwd);
	if (!head.ok) return bail(`could not re-fetch PR head SHA: ${head.err}`);
	if (head.out.trim() !== payload.headSha) return bail(`PR head changed since prepare (reviewed ${payload.headSha.slice(0, 12)}, now ${head.out.trim().slice(0, 12)}) — restart the review; NOT posting`);
	...
	const res = exec("gh", ["pr", "review", String(payload.pr), "--repo", `${payload.owner}/${payload.repo}`, flag, "--body", payload.body], cwd);
	if (!res.ok) return bail(`gh pr review failed: ${res.err}`);

	fs.writeFileSync(marker, `${JSON.stringify({ payloadHash: hash, headSha: payload.headSha, event: payload.event, at: new Date().toISOString() }, null, 2)}\n`);
