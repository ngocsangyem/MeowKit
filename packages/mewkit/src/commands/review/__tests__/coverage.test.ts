...
const smallImpact = (scoutRequired = false) => ({
	scoutRequired,
...
const fullReads = (scoutRequired = false): Read[] =>
	roster(scoutRequired).entries.flatMap((e) => e.expectedReads.map((target) => ({ reviewer: e.id, target })));
...
describe("reviewCoverage — gap detection", () => {
	...
	it("flags a reviewer that never launched", async () => {
		seed(
			false,
			fullReads().filter((r) => r.reviewer !== "security"),
		);
		const r = await run();
	...
	it("flags a never-opened diff", async () => {
		seed(
			false,
			fullReads().filter((r) => !(r.reviewer === "correctness" && r.target === "diff.patch")),
		);
		const r = await run();
	...
	it("flags a never-read brief", async () => {
		seed(
			false,
			fullReads().filter((r) => !(r.reviewer === "tests" && r.target === "briefs/tests.md")),
		);
		const r = await run();
	...
	it("requires the scout report when the impact map demands it", async () => {
		...
		seed(
			true,
			fullReads(true).filter((r) => !(r.reviewer === "correctness" && r.target === "scout-report.md")),
		);
		const r = await run();
		expect(r.gaps).toContainEqual({
			type: "required-read-missing",
			reviewer: "correctness",
			target: "scout-report.md",
		});
	});
});
...
describe("reviewCoverage — evidence level", () => {
	...
	it("corroborates across CLI-normalized vs hook-raw target spellings", async () => {
		...
		seed(false, reads, [
			...new Set(reads.map((r) => (r.target.startsWith("briefs/") ? `${r.target}/` : `./${r.target}`))),
		]);
		const r = await run();
		expect(r.evidenceLevel).toBe("session-observed");
	});
