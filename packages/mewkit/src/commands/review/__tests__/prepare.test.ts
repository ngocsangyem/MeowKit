...
function makeExec(overrides: Partial<{ ghMeta: ExecResult; ghChecks: ExecResult; remotes: string }> = {}): Exec {
	return (file, args) => {
		if (file === "git" && args[0] === "remote")
			return OK(
				overrides.remotes ?? "origin\tgit@github.com:me/repo.git (fetch)\norigin\tgit@github.com:me/repo.git (push)\n",
			);
			...
			return OK(
				JSON.stringify({
					...
					manifest: {
						...
						session,
						nonce: "nonce123",
						worktreePath: `.worktrees/review-pr-${pr}-${session}`,
						pr: Number(pr),
						ref: `refs/mewkit/review-pr-${pr}-${session}`,
						headSha: "h".repeat(40),
						baseSha: "b".repeat(40),
						baseBranch: "main",
						baseRemote: "origin",
...
let cwd: string;
beforeEach(() => {
	cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mk-prepare-"));
});
afterEach(() => {
...
describe("reviewPrepare", () => {
	it("rejects an ambiguous bare number across multiple remotes", async () => {
		const exec: Exec = (file, args) => {
			if (file === "git" && args[0] === "remote")
				return OK("origin\tgit@github.com:me/fork.git (fetch)\nupstream\thttps://github.com/acme/repo.git (fetch)\n");
	...
	it("rejects an invalid target before touching git", async () => {
		const r = await reviewPrepare({
			target: "not a pr",
			cwd,
			json: true,
			deps: { exec: makeExec(), sessionId: () => "s1" },
		});
		expect(r.ok).toBe(false);
	});
	...
	it("captures a hashed immutable diff, impact map, and untrusted metadata", async () => {
		const r = await reviewPrepare({
			...
			cwd,
			json: true,
			deps: { exec: makeExec(), sessionId: () => "sess-a" },
		});
	...
	it("records context-unavailable when gh degrades, never silently omits", async () => {
		...
		const manifest = JSON.parse(
			fs.readFileSync(path.join(cwd, "tasks", "reviews", "sess-b", "manifest.json"), "utf-8"),
		);
