...
const cov = (report: object) => ({
	report,
	sha256: crypto.createHash("sha256").update(JSON.stringify(report)).digest("hex"),
});

let dir: string;
beforeEach(() => {
	dir = fs.mkdtempSync(path.join(os.tmpdir(), "mk-g2cov-"));
});
afterEach(() => {
...
function runValidator(verdict: unknown): number {
	...
	try {
		execFileSync("node", [VALIDATOR, p], { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
		return 0;
	} catch (e) {
	...
	it("passes a review PASS with complete, session-observed, hash-clean coverage", () => {
		expect(
			runValidator({
				review_session: "s",
				decision: "PASS",
				coverage: cov({ complete: true, evidenceLevel: "session-observed", gaps: [] }),
			}),
		).toBe(0);
	});
	...
	it("BLOCKS a PASS whose coverage is attested (exit 2)", () => {
		expect(
			runValidator({
				review_session: "s",
				decision: "PASS",
				coverage: cov({ complete: true, evidenceLevel: "attested", gaps: [] }),
			}),
		).toBe(2);
	});
	...
	it("BLOCKS when coverage is incomplete (exit 2)", () => {
		expect(
			runValidator({
				...
				coverage: cov({ complete: false, evidenceLevel: "attested", gaps: [{ type: "x" }] }),
			}),
		).toBe(2);
	});
	...
	it("BLOCKS a tampered coverage hash (exit 2)", () => {
		expect(
			runValidator({
				review_session: "s",
				decision: "PASS",
				coverage: { report: { complete: true, evidenceLevel: "session-observed" }, sha256: "BADHASH" },
			}),
		).toBe(2);
	});
