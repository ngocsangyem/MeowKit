// TS port of the MeowKit scanners so the wiki gate ships in dist/ (the repo-root
// .cjs files live outside this package's rootDir and would NOT be emitted).
// MIRROR these sources pattern-for-pattern when they change:
//   - plugin/hooks/lib/validate-content.cjs   → INJECTION_PATTERNS + validateContent
//   - plugin/hooks/lib/secret-scrub.cjs       → SECRET_PATTERNS + scrubSecrets
// Local additions cite web-to-markdown security.md Layer 5 (injection) / Layer 7 (secrets).
// A parity test (scanner-adapter.test.ts) asserts a shared payload set behaves identically
// in the .cjs source and this port.
