...
const HOST_TOOLS = new Set(["Read", "Grep", "Glob", "Bash", "Write", "Edit", "Task", "AskUserQuestion", "WebFetch", "WebSearch", "TaskCreate", "TaskGet", "TaskUpdate", "TaskList", "SendMessage", "Task(Explore)"]);
...
function sourceForAgents(claudeDir: string, ownershipByPath: Map<string, string[]>): AgentSource[] {
	return enumerateArtifacts(claudeDir).refs
...
export function artifactPatternsOverlap(left: string, right: string): boolean {
	...
	return (a.prefix.startsWith(b.prefix) || b.prefix.startsWith(a.prefix)) && (a.suffix.endsWith(b.suffix) || b.suffix.endsWith(a.suffix));
}
...
export function renderAgentViewCounts(claudeDir: string): string {
	...
	const count = (agentClass: string, routing: string) => agents.filter((entry) => entry.agentClass === agentClass && entry.routing === routing).length;
...
export function checkAgentConformance(repoRoot: string): CheckResult[] {
	...
	const agents = sourceForAgents(claudeDir, new Map(inventory.entries.filter((entry) => entry.type === "agent").map((entry) => [entry.path, entry.ownedArtifacts ?? []])));
	...
	for (const agent of agents) {
		for (const line of agent.body.split("\n")) {
			...
			for (const token of line.matchAll(/`([A-Za-z][A-Za-z0-9]*(?:\([^`]+\))?)`/g)) {
				...
				if (knownTools.has(tool) && !agent.tools.has(tool)) {
					results.push(fail(`Agent tools: ${agent.path}`, `body instructs \`${tool}\` but frontmatter does not grant it`));
				}
			}
		}
		...
		if (/\bend\b[^\n]*\bstatus block\b/i.test(agent.body) && !agent.body.includes(STATUS_REFERENCE)) {
			results.push(fail(`Agent status reference: ${agent.path}`, "status-block instruction does not use the canonical A1 reference"));
		}
	}
		...
		for (let j = i + 1; j < writers.length; j += 1) {
			for (const left of writers[i].artifacts.filter((value) => value.includes("*"))) {
				for (const right of writers[j].artifacts.filter((value) => value.includes("*"))) {
					if (artifactPatternsOverlap(left, right)) results.push(fail("Agent ownership overlap", `${writers[i].path} ${left} overlaps ${writers[j].path} ${right}`));
				}
			}
		}
	}
	...
	for (const entry of inventory.entries.filter((item) => item.type === "agent" && item.agentClass === "core-support")) {
		if (!entry.triggerOwner) {
			results.push(fail(`Agent trigger boundary: ${entry.path}`, "core/support agent needs trigger owner and negative boundary in description"));
		}
	}
	...
	for (const entry of inventory.entries.filter((item) => item.type === "agent")) {
		if (!modelIsResolvable(entry.model ?? "")) results.push(fail(`Agent model: ${entry.path}`, `model "${entry.model}" is not in the taxonomy`));
	}
