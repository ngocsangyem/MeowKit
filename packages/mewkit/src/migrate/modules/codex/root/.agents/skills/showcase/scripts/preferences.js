...
export const DEFAULT_PREFERENCES = Object.freeze({
  version: 1,
  screenshots: false,    // default off: screenshots need Puppeteer installed
  publishing: false,     // default off: local-only; see SKILL.md publish stub
...
export async function loadPreferences({ env = process.env } = {}) {
  ...
  try {
    ...
    return { path: preferencesPath, exists: true, preferences: normalizePreferences(parsed), warnings: [] };
  } catch (error) {
    if (error.code === 'ENOENT') {
      return { path: preferencesPath, exists: false, preferences: normalizePreferences(DEFAULT_PREFERENCES), warnings: [] };
    }
    return {
...
export async function savePreferences(preferences, { env = process.env } = {}) {
  ...
  return { path: preferencesPath, preferences: payload };
}

export async function resetPreferences({ env = process.env } = {}) {
  const preferencesPath = resolvePreferencesPath(env);
  try { await fs.unlink(preferencesPath); } catch (error) {
    if (error.code !== 'ENOENT') throw error;
  }
  return { path: preferencesPath, preferences: normalizePreferences(DEFAULT_PREFERENCES) };
}
...
export function parseSetArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const arg = argv[i];
    if (arg === '--no-screenshots') { updates.screenshots = false; continue; }
    if (arg === '--screenshots') { updates.screenshots = parseBoolean(argv[++i], '--screenshots'); continue; }
    if (arg === '--no-publishing' || arg === '--no-publish') { updates.publishing = false; continue; }
    if (arg === '--publishing' || arg === '--publish') { updates.publishing = parseBoolean(argv[++i], arg); continue; }
    if (arg === '--no-dual-language') { updates.languages = ['en']; continue; }
    if (arg === '--dual-language') { updates.languages = parseBoolean(argv[++i], '--dual-language') ? ['vi', 'en'] : ['en']; continue; }
    if (arg === '--language' || arg === '--languages') { updates.languages = parseLanguages(argv[++i]); continue; }
...
function withDerivedFields(result) {
  return {
    success: true,
    ...result,
    preferences: { ...result.preferences, dualLanguage: result.preferences.languages.length > 1 },
  };
}

function writeJson(data) { process.stdout.write(`${JSON.stringify(data, null, 2)}\n`); }
function writeError(error) { process.stderr.write(`${JSON.stringify({ success: false, error: error.message }, null, 2)}\n`); }

export async function main(argv = process.argv.slice(2), { env = process.env } = {}) {
  const command = argv[0] || 'get';

  if (command === 'path') { writeJson({ success: true, path: resolvePreferencesPath(env) }); return; }
  if (command === 'get') { writeJson(withDerivedFields(await loadPreferences({ env }))); return; }
  if (command === 'set') {
  ...
  if (command === 'reset') { writeJson(withDerivedFields(await resetPreferences({ env }))); return; }
  throw new Error(`Unknown command: ${command}`);
}

if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => { writeError(error); process.exit(1); });
}
