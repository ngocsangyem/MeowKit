---
name: "rule-scale-adaptive-rules"
description: "rule-scale-adaptive-rules"
---
...
When CSV returns `workflow=one-shot` AND the orchestrator confirms zero blast radius, Gate 1 (plan approval) may be bypassed — identical to `the fix skill` with `complexity=simple` behavior.
...
**Single source of truth:** the full decision matrix lives at `.agents/skills/autobuild/references/adaptive-density-matrix.md`. Do NOT duplicate the table here — it drifts. This rule references the matrix; the matrix is authoritative.
...
When `mk:scale-routing` returns `level=high` during a `the cook skill` run, the cook workflow MUST auto-enable `--strict` mode (full mk:evaluate) at Phase 4.5 — unless the user explicitly passes `--no-strict`.
