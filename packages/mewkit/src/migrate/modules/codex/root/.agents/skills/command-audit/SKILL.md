...
2. **Run `mk:cso` security audit** for each detected platform. Check for all blocked patterns defined in `rules/security-rules.md`:
...
- BLOCK findings must be resolved before any `the ship skill` command will execute