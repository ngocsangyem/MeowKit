...
- **TDD mode inheritance:** spawned agents read the same `.codex/session-state/tdd-mode` sentinel as the parent. If the parent invocation set TDD mode (via `--tdd`), child spawns observe the same mode automatically — no env var propagation required. Filesystem state persists across Bash tool calls and sub-task contexts.
