...
Use only if `.codex/scripts/bin/setup-workflow` is unavailable. The wrapper resolves to the venv-local binary; do NOT rely on global PATH.

## Authentication — three required env vars

In `.codex/.env` (gitignored):
...
A `SessionStart` hook (`.codex/hooks/jira-env-loader.sh`) validates `.codex/.env` presence and the 3 keys. It does NOT export anything — each hook runs in its own subprocess, so propagation is impossible. The wrapper handles per-call sourcing/export.

Status messages:
- `[mk:jira] env OK` — all 3 keys present
- `[mk:jira] <KEY> missing in .codex/.env` — specific key absent
- `[mk:jira] .codex/.env missing — see .codex/.env.example for setup`
...
| 2 | Auth | "jira-as: authentication failed — re-check `MEOW_JIRA_API_TOKEN` in .codex/.env" |
...
```
bash $(git rev-parse --show-toplevel)/.agents/skills/jira/scripts/jira-as.sh --version
```

Expected: prints version, exits 0. With env missing, errors with `:?` parameter-expansion hint pointing at `.codex/.env.example`.

## Troubleshooting

- **`jira-as not installed at <path>`** — run `.codex/scripts/bin/setup-workflow` (or use the manual fallback).
- **`MEOW_JIRA_API_TOKEN missing`** — copy `.codex/.env.example` to `.codex/.env` and fill the three vars.
