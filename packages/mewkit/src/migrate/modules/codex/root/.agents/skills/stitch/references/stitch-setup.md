...
The env hook loads `.codex/.env`; never hardcode the key. Install the local scripts with
`cd .agents/skills/stitch/scripts && npm install`. `tsx` comes from `scripts/package.json`.
