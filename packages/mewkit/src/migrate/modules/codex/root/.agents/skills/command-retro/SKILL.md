...
3. **Read topic files in `.meowkit/memory/` for patterns.** Load `fixes.json` first (fall back to `fixes.md` if absent), `review-patterns.json` first (fall back to `review-patterns.md` if absent), and `architecture-decisions.json` first (fall back to `architecture-decisions.md` if absent) — see `.agents/skills/rule-memory-read-rules.md`. Look for:
