  ...
  try {
    ...
    if (fs.existsSync(plansDir)) {
      const entries = fs.readdirSync(plansDir).filter((d) => {
        try { return fs.statSync(path.join(plansDir, d)).isDirectory(); } catch { return false; }
      });
      if (entries.length > 0) return entries.sort().pop()!;
    }
  } catch { /* no tasks/plans dir */ }
...
function generateDesignMd(html: string, screenId: string): string {
  ...
  const layoutType = hasNav && hasAside
    ? "sidebar-with-nav"
    : hasNav ? "top-nav-layout" : "single-column";
  ...
  const componentMatches = html.match(
    /<(section|nav|header|footer|main|aside|form|button|input|table|dialog|article|ul|ol)[\s>]/gi
  ) ?? [];
  const components = [...new Set(
    componentMatches.map((c) => c.match(/<(\w+)/)?.[1]?.toLowerCase() ?? "")
  )].filter(Boolean);
