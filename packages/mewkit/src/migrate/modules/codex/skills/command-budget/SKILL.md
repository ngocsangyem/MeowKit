...
Tracks and reports AI model usage costs. Reads from `.meowkit/memory/cost-log.json`.

### Execution Steps

#### Default (no flags)

1. Read `.meowkit/memory/cost-log.json`.
...
2. If confirmed: clear `.meowkit/memory/cost-log.json` (reset to empty array).
3. If not confirmed: abort.

### Output

Formatted table of cost data from `.meowkit/memory/cost-log.json`. No files are modified (except `--reset`).

### Related

For context window utilization and structural overhead, see `the context-audit skill`. `the budget skill` tracks USD only.