...
# the breakdown skill — Spec to stories + optional tickets + tech breakdown
...
- NOT for implementation planning → `the plan skill`
- NOT for single-ticket analysis or RCA → `mk:intake` or `mk:jira-analyst`

## Usage

```
the breakdown skill <source> [--source confluence|jira|github|linear|paste] [--project <KEY>]
```
