...
Phase B dispatches adversarial persona sub-task AFTER Phase A base reviewers complete. Personas receive the diff AND a summary of Phase A findings, so they go deeper — not wider.

## Assured-PR topology (roster-driven)

In assured PR mode the fan-out is scope-driven by the session roster
(`tasks/reviews/<id>/roster.json`, generated by `mewkit review prepare`; tier =
small/medium/large by diff size). Each `briefs/<id>.md` names the exact artifacts that
reviewer MUST read through `mewkit review read` (coverage, step-03, enforces it).

- **Whole-diff roles stay whole-diff.** `issue-fidelity`, `removed-behavior` (deleted
  exports / semantic replacement), `cross-file-tracer`, and `test-matrix` reason over
  the ENTIRE diff. A per-chunk **territory** reviewer cannot prove cross-file
  compatibility or that removed behavior was semantically replaced — never fold a
  whole-diff role into a territory.
- **Territory reviewers** (large tier) each own a bounded set of changed files.
- **Heavily-rewritten files** get three **invariant slices** — lifecycle/state,
  error/retry, config/early-return — one reviewer each (v1-confirmed cost; do not cut).
- When `impact-map.json` set `scoutRequired`, inject the saved `scout-report.md`
  section into Cross-file Tracer, Removed-behavior, Test Matrix, and Code Quality briefs.
...
3. Spawn a sub-task with this context:
...
5. Dispatch persona sub-task with context template above (anchor included per dispatch)
