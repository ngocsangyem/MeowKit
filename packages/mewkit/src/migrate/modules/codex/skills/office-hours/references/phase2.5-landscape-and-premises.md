...
```bash
grep -li "<keyword1>\|<keyword2>\|<keyword3>" .meowkit/memory/projects/*-design-*.md 2>/dev/null
```

If matches found, read the matching design docs and surface them:
- "FYI: Related design found — '{title}' by {user} on {date} (branch: {branch}). Key overlap: {1-line summary of relevant section}."
- Ask via stop and ask the user in chat: "Should we build on this prior design or start fresh?"

This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `.meowkit/memory/projects/`.
...
**Privacy gate:** Before searching, use stop and ask the user in chat: "I'd like to search for what the world thinks about this space to inform our discussion. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?"
...
Use stop and ask the user in chat to confirm. If the user disagrees with a premise, revise understanding and loop back.

---

# Phase 3.5: Adversarial Second Opinion (optional)
...
2. Dispatch via the Agent tool with the mode-appropriate prompt:
...
**Error handling:** If the sub-task fails or times out, the second opinion is informational — skip with "Second opinion unavailable. Continuing to Phase 4." and proceed.
...
4. **Cross-pass synthesis:** After presenting sub-task output, provide 3-5 bullet synthesis:
   - Where the primary reviewer agrees with the sub-task
   - Where they disagree and why
   - Whether the sub-task's challenged premise changes the recommendation

5. **Premise revision check:** If the sub-task challenged an agreed premise, use stop and ask the user in chat:

> The second opinion challenged premise #{N}: "{premise text}". Their argument: "{reasoning}".
> A) Revise this premise based on the feedback
