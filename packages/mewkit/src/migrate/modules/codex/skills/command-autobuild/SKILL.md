---
name: "command-autobuild"
description: "command-autobuild"
---

# the autobuild skill — Autonomous Product-Build Pipeline

## Usage

```
the autobuild skill [product description]
the autobuild skill --resume [run-id]
the autobuild skill --tier [MINIMAL|FULL|LEAN]
the autobuild skill --budget [amount-usd]
the autobuild skill --max-iter [N]
```

## Behavior

Runs `mk:autobuild` skill — the autonomous generator/evaluator build pipeline for green-field products. Preferred over `the cook skill` for product-level builds ("build me a kanban app").

Adaptive scaffolding density per model tier:
- **MINIMAL** (Haiku) — short-circuits to `the cook skill`
