---
name: "rule-orchestration-rules"
description: "rule-orchestration-rules"
---

# Orchestration Rules

These rules apply when the agent spawns sub-task or coordinates parallel work.
...
- **Explicit `the cook skill` invocation** → `mk:cook` owns the full pipeline. `mk:workflow-orchestrator` does NOT activate for the remainder of the session.
...
When spawning a sub-task, ALWAYS include in the prompt:

1. **Work context path**: the git root of the files being worked on
2. **Plan reference**: path to the active plan file
3. **File ownership**: which files this sub-task may modify
...
Before spawning any sub-task, verify you have included:
...
- [ ] **File ownership** — glob patterns for files this sub-task may modify
- [ ] **Acceptance criteria** — how to verify the sub-task's work is complete
- [ ] **Constraints** — what the sub-task must NOT change or touch
...
For non-trivial delegations, prefer `the delegate skill` to assemble the prompt from the template above. The command wizards through the 7 required fields, runs an injection-pattern guard on each user-provided field, and emits a ≤200-token prompt block for review before you paste into the Task tool. Compatible with all inner harnesses via `--silent` mode.
...
Boundary violations show up as: sub-task does the wrong thing despite a "clear" prompt, then re-reads your conversation to "understand what you really meant." Re-reading is the symptom — the prompt was too thin OR too thick at the boundary.
...
- Do NOT assume rule files are auto-loaded into sub-task context. If the inner harness has a small auto-injection cap (e.g. a 32 KB `AGENTS.md`), delegated prompts must stay lean.
- Pass required rules content EXPLICITLY in the Task prompt if the inner harness will not inject it.
- Use `mk:delegate --silent` when interactive prompts via `stop and ask the user in chat` are not supported by the inner harness — the silent path emits the template without the wizard.
...
|---|---|
| `SubagentStart` hook for sub-task context injection | Hooks fire inside sub-task → infinite-loop risk (verified in `HOOKS_INDEX.md`). |
| `PreToolUse:Task` hook validating sub-task prompts | Violates outer-harness principle. Could break legitimate Task calls. Make-correct-path-easy beats block-incorrect-path. |
...
Each agent or sub-task MUST own distinct files — no overlapping edits.
...
sub-task receive only the context they need.
...
## sub-task Completion Handling

sub-task MUST end with the status block defined in `agent-conduct.md` A1.
