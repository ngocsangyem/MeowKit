...
## Rule 2: Persistent Skill State MUST Use the project environment

When a skill needs to persist data across sessions (append-only logs, JSON state, SQLite databases, history files), it MUST write to `the project environment` — NOT to the skill's own directory.

WHY: Skill directories can be wiped on plugin upgrade; `the project environment` is stable.
...
Append to `the project environment/foo/log.json`, not `.agents/skills/foo/log.json`.
