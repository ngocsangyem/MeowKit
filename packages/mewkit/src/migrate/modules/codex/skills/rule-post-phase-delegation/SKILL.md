---
name: "rule-post-phase-delegation"
description: "rule-post-phase-delegation"
---
...
| `mk:autobuild`                 | Step 5 pre-escalation (before stop and ask the user in chat on iter cap) | **foreground** |
...
- Environment variable `MEOWKIT_PM_AUTO=off` (user opt-out; `the status skill` still works)
...
about to decide ship vs abort via stop and ask the user in chat. Current delivery
...
User-invoked `the status skill` is ALWAYS honored regardless of this env var.
...
Skills NOT in this list do NOT delegate to PM. User-invoked `the status skill`
