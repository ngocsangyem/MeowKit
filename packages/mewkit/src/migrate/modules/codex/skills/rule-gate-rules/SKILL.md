---
name: "rule-gate-rules"
description: "rule-gate-rules"
---
...
`the fix skill` with complexity=simple bypasses Gate 1. The fix IS the plan — the scope is small enough that a separate planning document adds overhead without value.
...
`.codex/hooks/lib/gate2-check.sh` enforces this at `git commit` / `push` / `merge`.
