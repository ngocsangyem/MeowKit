---
name: "rule-dead-weight-audit-rules"
description: "rule-dead-weight-audit-rules"
---
...
1. **Baseline:** run `the benchmark skill run --full`, capture the run id
2. **Disable** the component (env var flag, comment out hook registration, comment out a rule import)
3. **Re-run** `the benchmark skill run --full`, capture the disabled run id
4. **Compare:** `the benchmark skill compare {baseline-run-id} {disabled-run-id}` and read the `measured_delta` field (= `baseline_avg − disabled_avg`)
...
1. Bypass `mk:benchmark`. Run individual canary specs via `the cook skill <spec.md>` directly.
...
Two deterministic CLI surfaces feed it (no sub-task, no inner-harness hook, exit 0 — advisory):
