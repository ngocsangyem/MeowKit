...
The advisory subagent is not a substitute for Step 3 brainstorming or Step 4
planning: it supervises a decision the pipeline is already stuck on, and it never
produces a plan or a phase graph.

### Call

Dispatch the `athena` subagent in the foreground — `/athena`, or name it in the
request — and wait for its result. It inherits no conversation, so all five
fields go in the request text:
...
gate — including the planning gate on the Step 4 plan — and it is never counted
as verification. Verification stays with Step 6 tests and the Step 7 review verdict.

Foreground delegate-and-wait is not yet confirmed against a live Cursor binary on
this surface. If the dispatch hangs, returns early, or the agent is not found,
print exactly `advice checkpoint unavailable in this runtime: <reason>` and
continue unsupervised. Never write a counsel packet inline and present it as the
advisory subagent's.
