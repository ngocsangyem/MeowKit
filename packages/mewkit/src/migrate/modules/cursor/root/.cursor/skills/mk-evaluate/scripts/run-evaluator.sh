...
rubric_loader="$repo_root/.cursor/skills/rubric/scripts/load-rubric.sh"
...
# mk:evaluate — Dispatch Instructions
...
delegate a sub-task (
  ...
  prompt="
    Read .cursor/skills/evaluate/workflow.md and follow it.
...
\`\`\`
.cursor/skills/evaluate/scripts/validate-verdict.sh tasks/reviews/{date}-{slug}-evalverdict.md
\`\`\`
