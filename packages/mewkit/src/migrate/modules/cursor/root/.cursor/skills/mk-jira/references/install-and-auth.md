...
This triggers `packages/cli/src/core/dependency-installer.ts` to read every per-skill `requirements.txt` (discovered by `skills-dependencies.ts:findRequirementsFiles()`) and `pip install` into `.cursor/skills/.venv`. The jira-as binary lands at:

```
.cursor/skills/.venv/bin/jira-as
```

## Manual fallback

```
.cursor/skills/.venv/bin/pip install jira-as 'jira-as[keyring]'
```

Use only if `setup-workflow` is unavailable. The wrapper resolves to the venv-local binary; do NOT rely on global PATH.
...
The wrapper `.cursor/skills/jira/scripts/jira-as.sh` translates these to jira-as's native `JIRA_API_TOKEN`, `JIRA_EMAIL`, `JIRA_SITE_URL` and exports `JIRA_OUTPUT=json` as the default.

The legacy `MEOW_*` prefix (vs. modern `MEOWKIT_*`) is a deliberate exception per migration directive.

## Hook validation

An environment-validation step, run via the project's configured hooks where available, validates `.env` presence and the 3 keys. It does NOT export anything — each invocation runs in its own subprocess, so propagation is impossible. The wrapper handles per-call sourcing/export.

Status messages:
- `[mk:jira] env OK` — all 3 keys present
- `[mk:jira] <KEY> missing in .env` — specific key absent
- `[mk:jira] .env missing — see .env.example for setup`
...
| 2 | Auth | "jira-as: authentication failed — re-check `MEOW_JIRA_API_TOKEN` in .env" |
...
```
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh --version
```

Expected: prints version, exits 0. With env missing, errors with `:?` parameter-expansion hint pointing at `.env.example`.

## Troubleshooting

- **`jira-as not installed at <path>`** — run `setup-workflow` (or use the manual fallback).
- **`MEOW_JIRA_API_TOKEN missing`** — copy `.env.example` to `.env` and fill the three vars.
