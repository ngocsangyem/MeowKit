...
LOOP:
  ...
  2. Run .cursor/skills/cook/scripts/validate-gate-2.sh (if available)
...
Before presenting Gate 2 (either cycle), the workflow evidence index must be complete: run `validate-gate-2.sh` (authoritative structural guard) AND this project's workflow-evidence validator for the cook phase (completeness mirror; concrete script path depends on what's installed). Surface both; neither approves — they gate on structure/completeness only. See `workflow-steps.md` → Workflow Evidence Index.
