...
Read the last N records from `.meowkit/memory/trace-log.jsonl` via venv python. Filter by schema_version. Create the analysis directory.
...
```bash
TRACE_LOG=".meowkit/memory/trace-log.jsonl"
if [ ! -f "$TRACE_LOG" ]; then
...
Use `.cursor/skills/.venv/bin/python3` (per `rules/` — no `jq` dependency):
