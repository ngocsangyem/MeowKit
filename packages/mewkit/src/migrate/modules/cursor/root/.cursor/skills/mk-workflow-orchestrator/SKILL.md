...
<!-- Canonical source: references/workflow-phases.md (no separate workflow.yaml ships in this bundle) -->
...
`references/workflow-phases.md` is the sole source for phase sequence, gates, leads, and required outputs. This adapter must not restate phase details — load that reference for execution.
...
3. **Execute the canonical lifecycle** — load `references/workflow-phases.md` and follow it directly; do not duplicate or override its phase ordering, TDD rules, required outputs, or gates.
