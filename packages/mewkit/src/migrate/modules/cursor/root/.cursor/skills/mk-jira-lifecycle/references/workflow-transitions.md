...
```bash
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh lifecycle transitions PROJ-123
```

(verify the exact verb name with `--help` — older jira-as versions may surface this under a different sub-verb)

Use either `--to "Status Name"` or `--id <transition_id>` on the subsequent `lifecycle transition` call. Never assume a transition exists — the project may use a custom workflow.
