...
Optional team-specific routing enhancement via `.cursor/product-areas.yaml`.
When the file does not exist, scale-routing operates in CSV-only mode — no degradation.

## Contents

- [When to Use](#when-to-use)
- [Creating `.cursor/product-areas.yaml`](#creating-cursorproduct-areasyaml)
- [Schema](#schema)
...
## Creating `.cursor/product-areas.yaml`

Place the file at the root of your project's `.cursor/` directory:
...
| `paths` | Yes | string[] | File-glob patterns matching source files for this area |
...
1. Check if `.cursor/product-areas.yaml` exists in the project root
