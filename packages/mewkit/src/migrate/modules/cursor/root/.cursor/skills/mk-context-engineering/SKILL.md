...
description: "Runtime router for context decisions: read-more/stop/ask/assume/delegate, minimal read-set, compaction timing. Model: Write/Select/Compress/Isolate. NOT for agent-tree overhead audit (context-audit)."
---
...
**Boundary vs `mk:context-audit`:** that skill audits *structural* agent-tree overhead
...
structural agent-tree setup or this session's runtime context? — then route.
...
- Auditing agent-tree token overhead → `mk:context-audit`.
...
- `mk:context-audit` — use instead for structural agent-tree overhead (tokens/% statically loaded).
