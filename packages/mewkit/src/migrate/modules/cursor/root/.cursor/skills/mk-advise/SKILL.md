...
Default: the isolated `advisor` custom agent, which
runs on the strongest-available-model advisory profile.

The harness has **no sub-task pause/resume** (`orchestration-rules.md` → Rejected
Patterns), so an interview cannot be one long-lived sub-task. Each turn is a
...
**Disclosed fallback:** if the strongest-available-model profile or the relay is unavailable, run
