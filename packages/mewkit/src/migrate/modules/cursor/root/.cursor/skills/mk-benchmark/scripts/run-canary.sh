...
# Reqs:   Bash 3.2+, .cursor/skills/.venv/bin/python3
#
# Quick tier (default): 5 tasks under .meowkit/benchmarks/canary/quick/, ≤$5
# Full tier (--full): quick + 1 heavy task under .meowkit/benchmarks/canary/full/, ≤$30
...
if [ "${1:-}" = "check-cap" ]; then
  ...
  PY=".cursor/skills/.venv/bin/python3"; [ -x "$PY" ] || PY="python3"
...
BENCH_DIR=".meowkit/benchmarks"
RESULTS_DIR="$BENCH_DIR/results"
mkdir -p "$RESULTS_DIR"

PY=".cursor/skills/.venv/bin/python3"
[ -x "$PY" ] || PY="python3"
...
if [ -x ".meowkit/hooks/append-trace.sh" ]; then
  RUN_MODEL="${MEOWKIT_MODEL_HINT:-the project environment:-unknown}}"
  bash .meowkit/hooks/append-trace.sh "benchmark_result" \
...
echo "         bash .cursor/skills/benchmark/scripts/run-canary.sh check-cap $LEDGER_FILE $COST_CAP"
...
echo "     bash .meowkit/hooks/append-trace.sh benchmark_result \"\$(cat $RESULT_FILE)\""
echo "═══════════════════════════════════════════════════════════════"
