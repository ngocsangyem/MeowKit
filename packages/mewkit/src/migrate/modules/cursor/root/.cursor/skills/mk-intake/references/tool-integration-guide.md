...
Setup:
```bash
# Install jira-as into the workflow venv (auto-handled by setup-workflow)
setup-workflow

# Populate the project's .env with the three MEOW_JIRA_* vars
cp .env.example .env
...
`mk:intake` triages the ticket; structured execution is delegated to the `mk:jira-*` family. See `.cursor/skills/jira/references/install-and-auth.md` for full setup details.
