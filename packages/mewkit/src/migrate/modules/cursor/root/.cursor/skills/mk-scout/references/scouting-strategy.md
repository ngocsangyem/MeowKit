...
For each segment, spawn:
  ...
  prompt: |
    ...
    (Per `AGENTS.md` (Data & injection boundary) Rule 4.)
...
Append the A1 status block exactly as defined in `AGENTS.md` (Subagent status protocol) after the report template.
...
- Each Explore agent has its own context window (~200K tokens), so its scan never
  ...
  the built-in Explore agent is a cheap, read-only tier (Write/Edit denied by the
...
If a downstream agent needs to read files found by scout, use the host runtime's native file-read capability, chunked by pagination when the host supports it — not bash workarounds:

```
Small file (<500 lines):   read the whole file
Medium file (500-1500):    read the first ~500 lines, then the next ~500-line chunk
Large file (>1500):        read in ~500-line chunks, advancing the start offset each time
```

Never `cat` or `sed` for file reading. The host's native read capability handles encoding, permissions, and line numbers natively.
