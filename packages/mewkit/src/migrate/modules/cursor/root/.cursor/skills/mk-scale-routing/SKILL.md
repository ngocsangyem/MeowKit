...
7. **Optional:** Check `.cursor/product-areas.yaml` if exists — merge area keywords and paths (see `references/product-area-config.md`)
...
| `product_area` | string | Area name from `.cursor/product-areas.yaml` (omitted if no YAML) |
