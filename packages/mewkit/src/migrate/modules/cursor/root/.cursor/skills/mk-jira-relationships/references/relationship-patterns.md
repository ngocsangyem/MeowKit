...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh relationships get-blockers STORY-100 --recursive --depth 5
...
```bash
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh relationships get-blockers PROJ-100 --recursive --output tree
...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh relationships bulk-link \
...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh relationships clone PROJ-100

# Preserve structure
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh relationships clone PROJ-100 --include-links --include-subtasks

# Different project
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh relationships clone PROJ-100 --to-project OTHER
```
...
> Before emitting any Mermaid block, Read `.cursor/skills/preview/references/mermaid-essentials.md`.
...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh relationships get-dependencies PROJECT-ROOT --output mermaid

# Better - focused on current sprint
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh relationships get-dependencies SPRINT-EPIC --output mermaid
```
...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh relationships get-dependencies RELEASE-EPIC --output dot > weekly-deps.dot
