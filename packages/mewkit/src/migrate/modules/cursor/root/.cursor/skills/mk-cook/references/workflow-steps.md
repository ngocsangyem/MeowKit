...
- [Phase 4: Review](#phase-4-review)
  - [Code Review (MANDATORY sub-task)](#code-review-mandatory-sub-task)
  - [GATE 2](#gate-2)
...
**Task Tracking Fallback:** tracked-task creation/update/list may be CLI-only — unavailable in some GUI editor extensions. If unavailable, use a plain to-do list for progress tracking. Plan files remain source of truth.
...
6. Create a tracked task for each workflow phase, with each phase blocked-by its predecessor
...
**Before presenting Gate 1 in non-auto modes**, run `.cursor/skills/cook/scripts/validate-gate-1.sh <plan-path>` as a structural pre-check. If exit ≠ 0, surface the blocker message to the user as a preflight WARNING above the Gate 1 prompt — then still present the prompt. In non-auto modes the script is **advisory**: the user retains override authority and may approve a plan that fails structural check. This preserves the "no hidden execution transitions" constraint.

Present plan summary. Use `stop and ask the user in chat` (header: "Gate 1"):
...
2. Spawn `tester` sub-task via `mk:testing`:
   ```
   delegate a sub-task (subagent_type="tester", prompt="Write failing tests for [plan-path] acceptance criteria. Tests must RUN (no syntax errors) and FAIL (not pass). Use mk:testing red-green-refactor reference.", description="Write failing tests")
   ```
...
**Pre-check (TDD gate — opt-in):** Before writing any implementation code, verify the RED-phase gate — enforced by the project's configured hooks where available, or performed as a manual check otherwise. TDD mode is ON when:

- `MEOWKIT_TDD=1` env var is set (CI / shell rc), OR
- `.meowkit/state/tdd-mode` sentinel file contains `on` (written when `--tdd` is detected)

When ON, verify failing tests exist before implementing. If tests pass (no red), implementation is BLOCKED — go back to Phase 2.
When OFF (default), proceed without the RED-phase gate.

Note: This is a phase-start check, not a per-file-write gate (too expensive as a per-edit check).

1. **List existing tasks first** — check for tasks already hydrated by the planning skill
2. If tasks exist → pick them up. If not → create a tracked task per plan phase with priority + metadata
3. Update each task's status to `in_progress` immediately when starting
...
6. If tests fail after implementation:
   ...
   - After 3 failures → spawn `researcher` sub-task via `mk:investigate`:
     ```
     delegate a sub-task (subagent_type="researcher", prompt="Analyze test failures using mk:investigate + mk:sequential-thinking: [output]. Root cause analysis.", description="Debug test failures")
     ```
...
- Update each task's assignment + status per agent
...
```
delegate a sub-task (subagent_type="developer", prompt="Run the simplify skill on the Phase 3 output. Reduce complexity without breaking tests. All tests must still pass after simplification.", description="Simplify before review")
```
...
### Code Review (MANDATORY sub-task)
...
If `red-team-findings.md` exists, load its contents and pass as additional context to the reviewer sub-task. This file contains plan-level adversarial findings (risks, attack surfaces, design concerns) that the reviewer should cross-reference against the implementation to verify they were addressed.

Spawn `reviewer` sub-task:

```
delegate a sub-task (subagent_type="reviewer", prompt="Review changes for [phase]. Return: score (X/10), critical_count, warnings list, suggestions list. Check: security, performance, YAGNI/KISS/DRY. [If red-team-findings.md loaded]: Also cross-reference the attached plan-level adversarial findings against the implementation — note which findings were addressed and which remain open.", description="Code review")
```
...
- `.cursor/skills/cook/scripts/validate-gate-2.sh` — the authoritative Gate 2 structural guard (no FAIL dimensions, side-effect addendum present).
- Run this project's workflow-evidence validator for the cook phase (concrete script path depends on what's installed; treat it as an interface, not a specific implementation) — evidence-completeness mirror (blocks on missing `cookContract` dims, empty `verification.commands`, `gate2-approved-without-verdict`, side-effects-without-addendum). It MIRRORS the gate script's status; it never replaces it and never approves. Pass `--plan-input` when the cook input was an existing `plan.md` / `phase-*.md` path.
...
2. **Spawn evaluator sub-task:**
   ```
   delegate a sub-task (
     subagent_type="evaluator",
     prompt="Run the evaluate skill on this build.
...
```
delegate a sub-task (subagent_type="shipper", prompt="Ship changes: 1) Run pre-ship.sh (test+lint+typecheck), 2) Conventional commit, 3) Create PR if on feature branch, 4) Verify CI passes, 5) Document rollback steps. Use mk:ship skill.", description="Ship + PR")
```

Note: `shipper` is the orchestrator — it invokes `git-manager` internally for raw git operations (commit, push). Cook never spawns `git-manager` directly; that is shipper's responsibility. See SKILL.md Required sub-task table.
...
Three mandatory sub-task in parallel:

1. **Plan sync-back:**

   Sync-back algorithm authority: `.cursor/skills/plan-creator/references/task-management.md` (Sync-Back Algorithm).

   ```
delegate a sub-task (subagent_type="planner", prompt="Run full sync-back for [plan-path] using the Sync-Back Algorithm in .cursor/skills/plan-creator/references/task-management.md. Report unresolved mappings if any todo cannot be matched to a phase.", description="Plan sync-back")
   ```

   If sync-back leaves every non-abandoned phase todo checked, archive the plan with `npx mewkit plan archive {plan-dir}` — it stamps `status: completed` across plan.md + every phase file (and `.plan-state.json`, preserving its visual block), then moves the whole plan directory to `tasks/plans/archive/{plan-name}/`. Do this in Phase 6; do not wait for `the ship skill`.

2. **Docs update:**

   ```
   delegate a sub-task (subagent_type="documenter", prompt="Evaluate docs impact for changes in [files]. Update docs/ if needed. State: Docs impact: [none|minor|major]", description="Update docs")
   ```
...
4. Mark all session tasks complete after sync-back

5. **Terminal wiki handoff (advisory, fail-open):** after sync-back, docs, and memory writes complete, optionally hand the implemented plan off to the wiki per `.cursor/skills/wiki/references/terminal-handoff-advisory.md`. Use the status report at `{plan-dir}/status-reports/<YYMMDD>-status.md` if present, else the synced `plan.md`. Resolve the slug (env `MEOWKIT_WIKI_SLUG` → the sole `tasks/wikis/<slug>/wiki.json` → else skip + print the command). Advisory only — never blocks, never approves; do not add `wiki reindex`:
...
1. **Phase-file checkboxes** — flip that phase's `[ ]` → `[x]` items via the Sync-Back Algorithm in `.cursor/skills/plan-creator/references/task-management.md` (same status enum: `pending | in_progress | completed`).
...
- If delegate a sub-task () tool calls = 0 at end of workflow → INCOMPLETE
