...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh time estimate PROJ-123 --original "2d"

# Set remaining estimate
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh time estimate PROJ-123 --remaining "1d 4h"

# Set both together (recommended due to JRACLOUD-67539)
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh time estimate PROJ-123 --original "2d" --remaining "1d 4h"
```
...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh time estimate PROJ-123 --original "2d"  # For a 3-point story
```
...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh time estimate PROJ-123 --original "1d 4h"
```
...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh time log PROJ-123 --time 2h

# Leave unchanged - time logged but estimate stays same
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh time log PROJ-123 --time 2h --adjust-estimate leave
...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh search query \
