...
#
# Usage:
#   bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/fetch-workflow.sh <ISSUE_KEY>
...
WRAPPER="$ROOT/.cursor/skills/jira/scripts/jira-as.sh"
...
if [ $RC -eq 0 ] && [ -n "$WF_JSON" ]; then
  ...
  # Write workflow definition (markdown with statuses + transitions tables)
  {
    ...
    echo
    echo "<!-- managed by .cursor/skills/jira/scripts/fetch-workflow.sh — manual edits will be overwritten on re-fetch -->"
    echo
