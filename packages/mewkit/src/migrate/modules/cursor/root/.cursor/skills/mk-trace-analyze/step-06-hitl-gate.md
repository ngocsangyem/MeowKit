...
Present each suggestion individually via `stop and ask the user in chat`. Approved suggestions land in `suggestions.md`. Modified suggestions get the user's revised version. Rejected suggestions land in `rejected.md` with the reason. **No auto-apply, ever.**
...
### 6b. For each suggestion, present via stop and ask the user in chat
...
**One suggestion per stop and ask the user in chat call.** No bulk-approve. No "approve all remaining."
...
| Modify | Ask follow-up `stop and ask the user in chat` for the revised text; append revised version to `suggestions.md` |
...
{If A > 0}: Generate a follow-up plan via:
  mk:plan-creator --hard "Apply trace-analyzer suggestions from {analysis_dir}"
...
If ≥1 suggestion approved AND user opts in via a final stop and ask the user in chat:

```
stop and ask the user in chat(
  question: "{A} suggestion(s) approved. Generate a follow-up plan now via mk:plan-creator?",
  options: [
    { label: "Yes — generate plan", description: "Spawn mk:plan-creator --hard with the approved suggestions" },
