...
```
the chom skill <github-url|web-url|local-path|description> [feature] [--compare|--copy|--improve|--port] [--lean|--auto]
```
...
| Local path (`./`, `../`, `/`)            | Starts with `.` or `/` | Direct read/file glob                                  |
| Freeform text                            | No URL or path         | `researcher` agent via delegate a sub-task () (WebSearch)              |
...
Escalation: if the feature has ≥3 architectural layers OR stateful workflows, emit handoff text: "Complex flow detected. Before Phase 4, trace via the sequential-thinking skill, then return to chom." Do NOT auto-invoke any skill.
...
To implement, run:
  the plan-creator skill "Replicate [feature] from [source]"

To bias future chom analysis toward a specific adaptation depth, re-invoke with:
  the chom skill [source] --copy | --improve | --port
```
...
This skill emits handoff text. It does NOT invoke any other skill mid-flow — including `the plan-creator skill`, `the brainstorming skill`, `the cook skill`, `the sequential-thinking skill`.
...
- **API-surface queries use `the pack skill --compress`**, not chom. If the question is "what's the public API of library X" rather than "should we replicate it," pack's Tree-sitter signature extraction is the right tool. chom is for replication decisions, not API exploration.
- **3+ layers trigger sequential-thinking handoff**, not auto-invocation. Trace the flow via `the sequential-thinking skill`, then return to chom.
