...
Maps a `StoryRecord` to a Fibonacci point estimate, complexity verdict, inconsistency flags, optional split proposal, and (when the Agile rules are loaded) a DoR advisory. Reuses Fibonacci anchors from `.cursor/skills/jira/references/estimation-guide.md` — DO NOT drift from that table.
...
- Fibonacci anchors: `.cursor/skills/jira/references/estimation-guide.md`
