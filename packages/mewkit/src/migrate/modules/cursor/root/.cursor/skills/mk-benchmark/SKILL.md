...
- Total cost + duration
- Run ID written to `.meowkit/benchmarks/results/{run-id}.json` AND trace-log.jsonl
...
Each benchmark run writes a JSON dump to `.meowkit/benchmarks/results/{run-id}.json`:
...
bash .cursor/skills/benchmark/scripts/git-index-audit.sh [repo-path]
# comparison (adds local/remote-only counts + recursive diff status)
bash .cursor/skills/benchmark/scripts/git-index-audit.sh <local> <remote>
```

**Artifact location:** `.meowkit/benchmarks/audits/{YYMMDD-HHMMSS}-audit.json` — a SIBLING
