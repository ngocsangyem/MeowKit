...
Spawn the `evaluator` sub-task via `mk:evaluate` to actively verify the build against the rubric library. Returns a graded verdict (`PASS | WARN | FAIL`) with concrete runtime evidence.
...
2. **Generator built a path-style target (no live URL)** — pass the project path to `mk:evaluate`, which will boot it (step-02 of mk:evaluate workflow).
...
### 4c. Dispatch the evaluator sub-task
...
delegate a sub-task (
  ...
  prompt="
    Act as the evaluator role: an isolated, skeptic reviewer that actively verifies the
    build against the rubric library rather than trusting the generator's self-report.
...
MEOWKIT_RUN_ID="$run_id" .cursor/skills/autobuild/scripts/budget-tracker.sh add "$step_cost_usd" "step-04-iter-${iteration}"
MEOWKIT_RUN_ID="$run_id" .cursor/skills/autobuild/scripts/budget-tracker.sh check
```
