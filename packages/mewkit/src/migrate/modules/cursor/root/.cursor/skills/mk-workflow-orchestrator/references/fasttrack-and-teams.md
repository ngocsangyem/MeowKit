...
1. Read team config → 2. List unclaimed tasks → 3. Claim by updating task status → 4. Do work → 5. SendMessage lead → 6. Check for more

If Agent Teams not enabled: sequential execution with standard sub-task behavior.

### Teammate Operation Pattern

```
1. List tasks → find unclaimed ones
2. Update the claimed task's status to "in_progress"
3. Do work → update the task's status to "completed"
...
```
workflow:handoff  → Saves to .meowkit/state/workflows/[workflow-id]/
