...
Usage:
  ...
  python3 init-skill.py mk:my-feature --path .cursor/skills
...
def parse_skill_name(raw_name):
    """Parse and validate skill name. Enforce mk: prefix."""
    name = raw_name.strip().strip('"').strip("'")

    # Auto-prepend mk: if missing
    if not name.startswith("mk:"):
        name = f"mk:{name}"
...
def init_skill(skill_name, base_path):
    ...
    skill_dir = Path(base_path).resolve() / f"mk:{slug}"
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  python3 init-skill.py my-feature --path .cursor/skills")
        print("  python3 init-skill.py mk:data-validator --path .cursor/skills")
        print()
        print("Note: mk: prefix is auto-prepended if missing")
        sys.exit(1)

    skill_name = sys.argv[1]
    base_path = ".cursor/skills"
