...
When in doubt, run `bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh <group> <verb> --help` to confirm the actual signature for any specific command — `--help` is always authoritative.
...
- Resolves to venv-local `.cursor/skills/.venv/bin/jira-as`
- Sources `.env` and translates `MEOW_JIRA_*` → `JIRA_*`
...
- User override: `JIRA_OUTPUT=text bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh ...` still works.
...
| `--dry-run`       | show what _would_ happen without exec; surfaces `would_*` keys |
...
```
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh <group> <verb> --help
```
