...
- [Repomix Errors](#repomix-errors)
  ...
  - [Output file not in `.meowkit/cache/packs/`](#output-file-not-in-meowkitcachepacks)
...
- [Environment](#environment)
  - [`.meowkit/cache/packs/` not gitignored](#meowkitcachepacks-not-gitignored)
...
**Cause:** You invoked `the pack skill .` or a local path that resolves to the same git root as your working directory.
...
**Fixes:**
- For inbound context: use `the scout skill` instead.
...
### Output file not in `.meowkit/cache/packs/`
...
- Pack locally instead: `git clone --depth 1 ...` then `the pack skill <local-path>`.
...
- Re-read original files directly (file read / text search / glob) if you need them inbound.
...
### `.meowkit/cache/packs/` not gitignored

**Cause:** The skill install missed adding the entry.
**Fix:** Verify `.gitignore` (project-local) and `.gitignore.workflow` (shared template) both contain `.meowkit/cache/packs/`. Add manually if missing.
