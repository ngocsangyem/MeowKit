...
description: "Deprecated compatibility shim. Agent routing reads the canonical inventory under .cursor/agents/ directly; do not use this skill for agent selection."
---

# Lazy Agent Loader (Deprecated)

The former hand-maintained agent list was stale. Use `mk:agent-detector`, which reads
the canonical `.cursor/agents/` inventory and its routing rules. This compatibility
