...
**CLI boundary:** the installer = project infrastructure. `mk:bootstrap` = application code + full pipeline. Zero overlap. Never touch `.cursor/`.
...
- **Duplicating CLI init**: never generate `.cursor/` files — CLI handles that
