...
| Hook | `immediate-capture-handler.cjs` on Codex / Codex CLI (this bundle ships no hook of its own yet) |
| Hook event | Provider-specific: Codex / Codex CLI's is its own pre-prompt hook event; `Gemini CLI` maps to `BeforeAgent`; on Cursor / Kiro the editor-native prompt-submit hook applies (when configured). `mewkit migrate <target>` rewrites the event name per harness. |
...
This is the path agents MUST use. The `fix` skill is the reference implementation — Step 6 reads and appends to the canonical `fixes.json` (see `.cursor/skills/fix/SKILL.md`).
...
The previous instruction "agents should append observations using `##pattern:` / `##decision:` / `##note:`" was a documentation error. It read as if `##prefix:` was an agent-output API; the handler never fired for agent text. A live search across the agent and skill trees returned 39 files repeating that misleading template (23 agents + 16 skills, captured 2026-05-23). Those files now reference this contract.
...
- `immediate-capture-handler.cjs` (the Codex/Codex CLI user-keyboard-shortcut handler; see the Hook row above for the per-provider equivalent)
- `.cursor/skills/memory/references/session-capture.md` (Phase 6 Reflect routine — uses Path 2)
