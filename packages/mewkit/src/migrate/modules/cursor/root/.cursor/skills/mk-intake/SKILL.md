...
| Source | Adapter |
|---|---|
| Jira | `mk:jira-issue` (via `jira-as` wrapper, requires `MEOW_JIRA_*` in the project's `.env`) |
| Linear | Linear MCP (`codex mcp add linear`) |
| GitHub | `gh` CLI (`gh issue view <n>`) |
| Confluence | `confluence-as` wrapper directly (requires `MEOW_CONFLUENCE_*` in the project's `.env`); recognizes `*.atlassian.net/wiki/spaces/{KEY}/pages/{ID}/...` URLs and raw page IDs with explicit "from confluence" phrase. See `references/confluence-handoff-protocol.md`. |
...
1. Pre-flight: check `MEOW_CONFLUENCE_*` env vars are set. If missing, surface "Confluence not configured. Run the confluence-setup skill" and exit cleanly without attempting fetch.
...
4. The intake report's "Source Summary" section cites the Confluence URL + page metadata. The "Suggested Actions" section recommends: "For deep spec analysis (requirements, AC, gaps), run: `the confluence-spec-analyst skill <page-id>`". intake does NOT auto-invoke spec-analyst (no skill-to-skill chaining).
...
```bash
printf '%s' "$ticket_content" | node .cursor/skills/intake/scripts/sanitize-ticket.cjs
```
...
If the `mk:jira-*` family is available (jira-as installed via `setup-workflow` + the project's `.env` populated), load `references/jira-awareness.md` for metadata extraction and enhanced completeness scoring.
...
- Confluence → output to user only — Confluence write-back is out of scope; user reviews the intake report and (optionally) runs `the confluence-spec-analyst skill <page-id>` for deeper analysis
...
- Delegation example: `.cursor/skills/.venv/bin/python3 .cursor/skills/web-to-markdown/scripts/fetch_as_markdown.py "<url>" --wtm-accept-risk --caller mk:intake`
