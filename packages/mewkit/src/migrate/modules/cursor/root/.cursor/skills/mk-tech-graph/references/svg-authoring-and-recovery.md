...
printf '%s\n' \
  ...
  '</svg>' \
  | python3 .cursor/skills/tech-graph/scripts/generate-svg-from-lines.py /path/to/output.svg
```
...
Run `bash .cursor/skills/tech-graph/scripts/validate-svg.sh <svg-file>`. When available, also run:
