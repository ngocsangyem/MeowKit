...
Implementation entry adapter. `references/workflow-steps.md` is this bundle's lifecycle phase-sequence authority; this skill selects a profile, invokes the relevant owners, and presents the required gates. TDD is opt-in via `--tdd`.

## Usage

```
/mk-cook <natural language task OR plan path>
/mk-cook "Add user auth" --fast
/mk-cook "Build payment processor" --tdd       # Strict TDD enforced
/mk-cook tasks/plans/260329-feature/plan.md --auto
```
...
When `--tdd` is detected in the invocation, the cook skill MUST write `on` to `.meowkit/state/tdd-mode` via a shell tool call BEFORE any other workflow step:

```bash
mkdir -p .meowkit/state && echo on > .meowkit/state/tdd-mode
```

This sentinel file is read by downstream agents to detect TDD mode (this bundle ships no separate hook scripts — the sentinel is a plain file check). Without `--tdd`, the sentinel is absent and the workflow runs in default mode (Phase 2 optional, no RED-phase gate).
...
<!-- Canonical source: references/workflow-steps.md — this diagram renders the same lifecycle; -->
<!-- Workflow phase sequence: 0:Orient > 1:Plan > 2:Test > 3:Build > 3.5:Simplify > 3.6:Verify > 4:Review > 5:Ship > 6:Reflect -->
<!-- references/workflow-steps.md is the authoritative per-phase detail; this Mermaid diagram is the human-facing render. -->
...
This diagram is a convenience rendering only; phase details and transitions live in `references/workflow-steps.md`, the authoritative source.
...
| 4.5 Verify    | `browser-automation sub-task` or `HTTP verification tool` | Only if `--verify` flag (light browser check) |
| 4.5 Verify    | `evaluator` via `mk:evaluate`   | Only if `--strict` flag or auto-triggered     |

Concrete sub-task name and HTTP tool depend on installed skill set; cook treats both as interfaces, not specific implementations.
...
See `references/sub-task-patterns.md` for delegate a sub-task () invocation patterns.
...
- **Skipping mk:simplify before review**: Tests pass but code is still complex → run `the simplify skill` between Phase 3 and Phase 4 every time, no exceptions
...
- **Forgetting memory read/write**: Prior session learnings lost. Phase 0 reads canonical `.meowkit/memory/*.json` stores only for fix memory; Phase 6 writes canonical JSON stores and regenerates generated views when needed
- **sub-task patterns using Agent() not delegate a sub-task ()**: delegate a sub-task () enables tracking, blocking, and progress. Always use delegate a sub-task () for phases 2-6
