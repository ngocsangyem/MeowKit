...
_WORKFLOW_SPECIFIC: list[tuple[str, str]] = [
    ...
    # Pattern is case-insensitive (`(?i)`) — tool names below are lowercase so the
    # literal source never carries a bare capitalized tool-name token, while still
    # matching an attacker's mixed-case tool-name reference at runtime.
