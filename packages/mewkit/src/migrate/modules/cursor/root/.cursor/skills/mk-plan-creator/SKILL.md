...
Before producing a plan, plan-creator MUST be able to answer all 5 dimensions in concrete sentences (cook's exact-requirements contract — see `.cursor/skills/cook/SKILL.md` "Exact-Requirements Contract (Phase 1)"):
...
| `scripts/validate-plan.py`                         | Plan completeness validator. **Depends on PyYAML** (installed via this project's setup tooling into `.cursor/skills/.venv/`). Run via `.cursor/skills/.venv/bin/python3 scripts/validate-plan.py <plan.md>`. |
