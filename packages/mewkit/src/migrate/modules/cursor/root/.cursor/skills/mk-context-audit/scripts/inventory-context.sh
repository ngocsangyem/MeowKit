#!/usr/bin/env bash
# inventory-context.sh — Read-only walk of the project's agent tree (.cursor/ + .cursor/skills/); emits JSON byte/line counts.
#
...
# 2. agents (.cursor/agents/*.md)
inventory_agents() {
  local dir="$SCAN_ROOT/.cursor/agents"
  local first=1
  printf '['
...
inventory_skills() {
  local dir="$SCAN_ROOT/.cursor/skills"
  local first=1
  printf '['
...
# 4. rules (.cursor/rules/**/*.mdc)
inventory_rules() {
  local dir="$SCAN_ROOT/.cursor/rules"
  local first=1
  printf '['
  [ -d "$dir" ] && while IFS= read -r f; do
    [ -z "$f" ] && continue
  ...
  done < <(find "$dir" -type f -name '*.mdc' 2>/dev/null)
  printf ']'
}

# 5. MCP servers — count top-level entries under "mcpServers" in .cursor/mcp.json.
# Heuristic: total file bytes split evenly across detected servers.
inventory_mcp() {
  local f="$SCAN_ROOT/.cursor/mcp.json"
  printf '['
  if [ -f "$f" ]; then
...
printf '"agents_md_chain":'; inventory_agents_md_chain; printf ','
