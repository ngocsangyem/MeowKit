# Arguments and Mode Resolution

## User-invocable

When the user types `the cso skill`, run this skill.

## Arguments

- `the cso skill` — full daily audit (all phases, 8/10 confidence gate)
...
## Important: use the host runtime's code-search capability for all code searches

The bash blocks throughout this skill show WHAT patterns to search for, not HOW to run them. Use the host runtime's built-in code-search capability (which handles permissions and access correctly) rather than raw bash grep. The bash blocks are illustrative examples — do NOT copy-paste them into a terminal. Do NOT use `| head` to truncate results.
