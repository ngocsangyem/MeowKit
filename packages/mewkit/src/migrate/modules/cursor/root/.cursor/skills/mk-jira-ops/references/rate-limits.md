...
- Use request batching (not adopted; see jira-as `--help` per verb for built-in batching support) for parallel execution

### 4. Cache Aggressively

- Cache metadata (projects, users, fields) for hours
- Cache issue data for minutes
- Use cache warming via `bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh ops cache-warm --project PROJ` (`mk:jira-ops`) to pre-load data
...
- cache warming via `bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh ops cache-warm --project PROJ` (`mk:jira-ops`)
