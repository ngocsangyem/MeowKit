...
Loaded when the user has the `mk:jira-*` family available (i.e. `jira-as` installed via `setup-workflow` and the project's `.env` populated with `MEOW_JIRA_*`). When unavailable, skip this file entirely — mk:intake operates tool-agnostic.

## Metadata Extraction

Fetch the ticket via `mk:jira-issue` (or directly: `bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh issue get PROJ-123 --fields '*all'`). Extract these Jira-specific fields before scoring:
...
After analysis, append a `### Suggested Actions (mk:jira)` section to the report:

```markdown
### Suggested Actions (mk:jira)

- [ ] Evaluate complexity: the jira skill evaluate [TICKET-ID]
- [ ] Estimate story points: the jira skill estimate [TICKET-ID]  ← only if no story points set
