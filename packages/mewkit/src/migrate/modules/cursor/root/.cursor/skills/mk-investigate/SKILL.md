...
- **Reads memory:** canonical `.meowkit/memory/fixes.json` and `architecture-decisions.json`; fall back to matching generated Markdown views only when JSON is absent. See `AGENTS.md` (Memory).
...
- **Report-path hook**: a pre-write hook, enforced by this project's configured hooks when present, fails closed unless the destination is under `tasks/reports/**`.
...
- Delegation example: `.cursor/skills/.venv/bin/python3 .cursor/skills/web-to-markdown/scripts/fetch_as_markdown.py "<url>" --wtm-accept-risk --caller mk:investigate`