...
- The wrapper rejects credentials in `.mcp.json` and any non-Cloud site URL.

## Failure Modes

| Symptom | Likely cause | Remediation |
|---|---|---|
| Wrapper exits 2 | Credentials in `.mcp.json` instead of `.env` | Move `MEOW_CONFLUENCE_*` vars to `.env`; chmod 0600 |
...
| Wrapper exits 127 | Binary not installed | Run `setup-workflow` |
