...
> Patterns that trigger a warning in `mk:careful` mode.
> Referenced by: `.cursor/skills/careful/SKILL.md` and `.cursor/skills/careful/bin/check-careful.sh`
...
When the hook triggers a warning, the user is prompted to confirm. Overrides are logged to the canonical `.meowkit/memory/security-findings.json` store with timestamp and command for audit trail.
...
> Update this section when the agent hits new edge cases in practice.

- **Shell aliases bypass detection**: if the user has `alias del='rm -rf'`, the hook sees `del` not `rm -rf` → false negative. Document project-specific dangerous aliases in `.cursor/skills/rule-`.
