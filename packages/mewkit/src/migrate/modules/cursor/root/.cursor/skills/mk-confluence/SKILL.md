...
See `references/install-and-auth.md` for one-time setup (`setup-workflow` installs `confluence-assistant-skills` into `.cursor/skills/.venv` per `scripts/requirements.txt`; populate `.env` with the 3 `MEOW_CONFLUENCE_*` vars).
