...
**2. Failure-mode pass:** Dispatch a second sub-task with this prompt:
...
Each sub-task has fresh context — genuine independence catches what the primary reviewer is blind to.
...
```bash
workflow-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"codex","tier":"large","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
