...
Usage:
  ...
  python3 validate-skill.py .cursor/skills/my-feature
...
def validate_skill(skill_path):
    ...
    # Check 3: mk: prefix in name
    if has_fm:
        name_match = re.search(r"name:\s*(.+)", frontmatter)
        if name_match:
            name = name_match.group(1).strip().strip('"').strip("'")
            if name.startswith("mk:"):
                results.append(("mk: prefix", True, name))
            else:
                results.append(("mk: prefix", False, f"Name '{name}' missing mk: prefix"))
        else:
            results.append(("mk: prefix", False, "No name field in frontmatter"))
    else:
        results.append(("mk: prefix", False, "No frontmatter to check"))
