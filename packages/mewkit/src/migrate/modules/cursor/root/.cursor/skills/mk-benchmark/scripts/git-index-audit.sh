...
#
# Artifact: .meowkit/benchmarks/audits/{YYMMDD-HHMMSS}-audit.json  (override dir with
...
OUT_DIR="${MEOWKIT_AUDIT_OUT_DIR:-.meowkit/benchmarks/audits}"
mkdir -p "$OUT_DIR"
...
if [ -x ".meowkit/hooks/append-trace.sh" ]; then
  ...
  bash .meowkit/hooks/append-trace.sh audit_result "$TRACE_JSON" 2>/dev/null || true
fi
