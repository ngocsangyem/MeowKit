...
#    MINIMAL (cheap tier) = contract skipped, LEAN (capable tier) = contract optional
...
if [ ! -d "tasks/contracts" ]; then
  ...
  echo "  2. Set: export MEOWKIT_AUTOBUILD_MODE=LEAN  (skips contract for the capable tier)" >&2
  exit 1
fi
...
echo "  2. Set: export MEOWKIT_AUTOBUILD_MODE=LEAN  (capable-tier LEAN mode bypass)" >&2
exit 1
