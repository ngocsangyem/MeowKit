...
**Error handling:** If the sub-task fails or times out, the outside voice is informational — skip with "Outside voice unavailable. Continuing to outputs." and proceed.
...
For each substantive tension point, auto-propose as a TODO via stop and ask the user in chat:
...
```bash
workflow-review-log '{"skill":"plan-review-outside-voice","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"codex","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
