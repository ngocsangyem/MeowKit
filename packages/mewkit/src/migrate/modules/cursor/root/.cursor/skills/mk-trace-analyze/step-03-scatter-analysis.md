...
Spawn up to 3 `researcher` sub-task in parallel, each analyzing one batch against the error taxonomy. Each sub-task returns a list of detected patterns + occurrence counts.
...
- Read `references/error-taxonomy.md` once into working memory (each sub-task gets a copy)

### 3b. Spawn researcher sub-task in parallel

For each batch (1 to `$batch_count`), dispatch a researcher via the Task tool. **Send ALL delegate a sub-task () calls in a single message** to actually parallelize:
...
delegate a sub-task (
  ...
  prompt="
    Read the error taxonomy at:
      .cursor/skills/trace-analyze/references/error-taxonomy.md
