...
if (!process.env.STITCH_API_KEY) {
	...
	console.error("[X] Add to .env: STITCH_API_KEY=<your-stitch-api-key>");
	process.exit(1);
}
...
async function runGenerate(): Promise<void> {
	...
	if (!prompt) {
		console.error(
			"Usage: npx tsx stitch-api-call.ts generate <prompt> [--device mobile|desktop|tablet] [--variants <count>]",
		);
		process.exit(1);
	}
...
async function main(): Promise<void> {
	try {
		...
		if (err.code === "RATE_LIMITED") {
			...
			saveQuota(q);
			console.error(
				"[X] Daily quota exceeded (local tracker synced). Use mk:frontend-design or wait until midnight UTC.",
			);
