...
4. Generate with the relevant helper: run `python3 .cursor/skills/tech-graph/scripts/generate-from-template.py` for supported templates, or run `python3 .cursor/skills/tech-graph/scripts/generate-svg-from-lines.py <output.svg>` and pipe SVG lines to stdin for hand-authored output.
5. Run `bash .cursor/skills/tech-graph/scripts/validate-svg.sh <output.svg>`. If available, run `rsvg-convert <output.svg> -o /dev/null` as an additional check.
...
- Run `bash .cursor/skills/tech-graph/scripts/generate-diagram.sh -t <type> -s <style> -o <output.svg>` to validate an existing SVG and export PNG when available.
- Run `python3 .cursor/skills/tech-graph/scripts/generate-from-template.py <template> <output.svg> '<json>'` to render a built-in starter template from escaped JSON data.
- Run `python3 .cursor/skills/tech-graph/scripts/generate-svg-from-lines.py <output.svg>` to write hand-authored SVG lines safely from stdin.
- Run `bash .cursor/skills/tech-graph/scripts/validate-svg.sh <svg-file>` to validate XML, markers, attributes, and path data.
- Run `bash .cursor/skills/tech-graph/scripts/test-all-styles.sh` to batch-test styles and produce its report.
