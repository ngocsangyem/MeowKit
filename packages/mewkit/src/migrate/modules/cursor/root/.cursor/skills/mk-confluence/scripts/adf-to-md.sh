...
PY="$ROOT/.cursor/skills/.venv/bin/python3"
SCRIPT="$ROOT/.cursor/skills/confluence/scripts/adf_to_md.py"
WRAPPER="$ROOT/.cursor/skills/confluence/scripts/confluence-as.sh"

if [ ! -x "$PY" ]; then
  echo "[adf-to-md] venv not found at $PY - run \`setup-workflow\` first" >&2
  exit 127
fi
