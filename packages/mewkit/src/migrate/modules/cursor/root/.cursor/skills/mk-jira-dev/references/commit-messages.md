...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh dev parse-commits "PROJ-123: Fix login bug"

# Extract from git log
git log --oneline -10 | bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh dev parse-commits --from-stdin

# Filter by project
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh dev parse-commits "PROJ-123 OTHER-456" --project PROJ
```
