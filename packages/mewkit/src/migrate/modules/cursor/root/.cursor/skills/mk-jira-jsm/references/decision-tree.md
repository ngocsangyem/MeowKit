...
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh search query "project=SD AND type=Incident AND priority=High"

# Find SLA breaches
bash $(git rev-parse --show-toplevel)/.cursor/skills/jira/scripts/jira-as.sh search query "project=SD AND 'Time to resolution' breached"
```
