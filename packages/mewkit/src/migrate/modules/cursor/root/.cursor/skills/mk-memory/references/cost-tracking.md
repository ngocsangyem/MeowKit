...
- [`the budget skill` Command](#meowbudget-command)
...
- **On `the budget skill` command:** Display cost summary. `the budget skill` is a subcommand of `mk:memory`, not a standalone skill — invoke it while `mk:memory` is active: `the budget skill [--report|--alert N]`.
...
Print the estimate:
```
Task tier: [standard] | Model: [inherited from agent frontmatter]
```
...
- [ ] `the budget skill` output is formatted as a readable table