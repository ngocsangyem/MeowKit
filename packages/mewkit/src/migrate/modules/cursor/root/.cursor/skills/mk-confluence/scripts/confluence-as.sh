...
CONFLUENCE_AS="$ROOT/.cursor/skills/.venv/bin/confluence-as"
ENV_FILE="$ROOT/.env"
LOCAL_SETTINGS="$ROOT/.mcp.json"

if [ ! -x "$CONFLUENCE_AS" ]; then
  echo "[mk:confluence] confluence-as not installed at $CONFLUENCE_AS" >&2
  echo "[mk:confluence] Run: setup-workflow    (auto-installs from .cursor/skills/confluence/scripts/requirements.txt)" >&2
  exit 127
fi

# Refuse plaintext credential fallback in .mcp.json (security override of upstream default)
if [ -f "$LOCAL_SETTINGS" ] && grep -q "CONFLUENCE_API_TOKEN" "$LOCAL_SETTINGS" 2>/dev/null; then
  echo "[mk:confluence] credentials must live in .env, not .mcp.json" >&2
  echo "[mk:confluence] Move CONFLUENCE_* vars to .env (chmod 0600 recommended), then retry." >&2
  exit 2
fi
...
: "${MEOW_CONFLUENCE_API_TOKEN:?MEOW_CONFLUENCE_API_TOKEN missing — see .env.example}"
