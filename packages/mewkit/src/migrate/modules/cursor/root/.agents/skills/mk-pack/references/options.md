...
| `--output <path>` | auto-generated | `-o` | Override `.meowkit/cache/packs/...` default |

Auto-generated output path: `.meowkit/cache/packs/{YYYYMMDD-HHMM}-{slug}.{ext}` where slug is derived from the source (last URL segment or local dir basename).
...
- `--copy` (clipboard) — the clipboard isn't reliably exposed to agents in this environment
