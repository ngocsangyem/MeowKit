...
If the output is non-empty (working tree is dirty), **STOP** and use stop and ask the user in chat:

"Your working tree has uncommitted changes. the qa skill needs a clean tree so each bug fix gets its own atomic commit."
...
**If NO runtime detected** (no config files found): Use stop and ask the user in chat:
