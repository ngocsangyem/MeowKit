...
`immediate-capture-handler.cjs` hook persists them on the prompt-submission hook event (named differently per provider — see `capture-architecture.md`'s Hook row).
Agent output is NOT processed by the handler. Agents writing in this
skill use direct `Edit` per Steps 1–4 above. See
`.agents/skills/memory/references/capture-architecture.md`.
