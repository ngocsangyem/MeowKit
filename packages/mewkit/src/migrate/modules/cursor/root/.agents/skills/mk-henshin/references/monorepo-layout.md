...
Load during Phase 5 (Transformation Spec) so the plan captures the exact tree `the cook skill` will scaffold.
...
│       ├── Dockerfile
│       └── tsconfig.json
├── .agents/skills/<tool-name>/     # companion skill (staged for the kit marketplace)
├── docs/
│   ├── cli.md
│   ├── mcp.md
│   ├── architecture.md
...
- **One file per capability in `core/capabilities/` scales better than one big module.** Wildcard imports from `capabilities/*` keep `index.ts` clean and make tree-shaking work.