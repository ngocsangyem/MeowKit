...
- **Playwright is opt-in.** Default is static fetch only. JS-rendered pages return an error pointing to `setup-workflow --system-deps`. This is intentional — 200MB Chromium download is not worth the 5% of pages that need it.
...
- **Fetch persistence grows unbounded in v1.** Manual cleanup via `rm -rf .meowkit/cache/web-fetches/*`. v2 will add TTL auto-cleanup.
...
```bash
.agents/skills/.venv/bin/pip install -r scripts/requirements.txt
...
.agents/skills/.venv/bin/playwright install chromium  # ~200MB one-time
```
