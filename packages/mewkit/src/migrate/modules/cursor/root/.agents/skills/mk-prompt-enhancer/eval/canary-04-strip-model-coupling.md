...
- #10 Model-coupled framing — three vendor signals:
  - XML tags (`<context>`, `<task>`, `<requirements>`) — Codex idiom
  - "think step by step" line — Codex pre-4.5 idiom
  - "Reasoning: high" line — Gemini parameter

### Improvement Suggestions

- Strip ALL XML tags; replace with plain markdown `CONTEXT:` / `CONSTRAINTS:` / etc.
- Remove "think step by step" line (capable-tier models self-calibrate; on Codex it's noise).
...
- Output applies a Codex / GPT / Gemini overlay (rewrite must be model-agnostic).
...
overlays via `--model codex`; v1.2 strips coupling instead. This canary
