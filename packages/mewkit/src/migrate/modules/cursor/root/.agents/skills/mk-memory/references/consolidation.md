...
- [Pruning Topic Files (`the memory skill --prune`)](#pruning-topic-files-meowmemory---prune)
...
## Pruning Topic Files (`the memory skill --prune`)
...
- **Log what was changed** — append the summary to the markdown-native `quick-notes.md`
  (NOT a canonical-store view like `fixes.md`; per `AGENTS.md` (Memory) →
