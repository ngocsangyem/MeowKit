...
- **Skipping scout/research for unfamiliar codebases**: Planning without understanding current architecture produces plans that conflict with existing patterns → If unfamiliar codebase, always run mk:scout first
...
- **Security-sensitive plans need the cso skill before Security persona ships**: Plan red-team currently uses Assumption Destroyer + Scope Critic only. For auth/payments/PII plans, run `the cso skill` separately until Security Adversary persona is A/B tested and shipped
