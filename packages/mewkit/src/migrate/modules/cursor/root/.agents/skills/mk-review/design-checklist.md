...
> Lite design review for `mk:review` + `mk:ship` pre-landing. Source-level pattern detection — not rendered-output audit. For full visual audit, use an external tool.
