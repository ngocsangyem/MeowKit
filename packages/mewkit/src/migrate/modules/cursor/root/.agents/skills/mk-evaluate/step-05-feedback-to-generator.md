...
End the evaluator session with the structured status block per `agent-conduct.md` (A1):
...
This is the final step. Return to the caller (the harness skill or the user who invoked `the evaluate skill`).
