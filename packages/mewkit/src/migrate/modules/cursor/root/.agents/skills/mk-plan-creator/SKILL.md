...
| `scripts/validate-plan.py`                         | Plan completeness validator. **Depends on PyYAML** (installed via this project's setup tooling into `.agents/skills/.venv/`). Run via `.agents/skills/.venv/bin/python3 scripts/validate-plan.py <plan.md>`. |
