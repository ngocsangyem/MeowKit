...
Forks to the `story-sizer` custom agent. v1 ships paste-mode only: the user provides a markdown block of stories, the agent applies heuristics, and a Story Sizing Report is written to `tasks/reports/story-sizing-{YYMMDD}-{slug}.md`. No Jira side effects in default mode.
...
- DoR advisory only appears when `mk:agent-detector` loaded the agile story gates rule for the current session.