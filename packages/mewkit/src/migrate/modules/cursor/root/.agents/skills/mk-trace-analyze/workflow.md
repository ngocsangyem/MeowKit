...
1. `step-01-ingest.md` — Read last N records from `.meowkit/memory/trace-log.jsonl` via venv python; filter by `schema_version`
2. `step-02-partition.md` — Group records by `run_id`; split into ≤3 batches
3. `step-03-scatter-analysis.md` — Spawn 3 researcher sub-task in parallel; each gets one batch + the error-taxonomy reference
...
6. `step-06-hitl-gate.md` — Present suggestions one at a time via stop and ask the user in chat (Approve/Modify/Reject); write approved items to a draft plan
...
| `batch_findings` | step-03 | step-04 | list of researcher sub-task reports |
...
```
the trace-analyze skill [--runs N] [--pattern <name>]
    ↓
Step 1: Ingest
    ├── Read .meowkit/memory/trace-log.jsonl
...
Step 2: Partition
    ...
    └── If <3 run_ids, use fewer batches
         ↓
Step 3: Scatter
    ├── Spawn 3 researcher sub-task in parallel
...
Step 5: Suggestions
    ...
    └── Write to {analysis_dir}/suggestions-draft.md
         ↓
Step 6: HITL Gate
    ├── For each suggestion: stop and ask the user in chat [Approve / Modify / Reject]
