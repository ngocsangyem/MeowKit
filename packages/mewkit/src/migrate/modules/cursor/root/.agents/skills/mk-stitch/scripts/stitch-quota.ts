/**
 * stitch-quota.ts — Local quota tracker for Stitch daily credits.
 *
 * Usage:
 ...
 * only manages a local JSON file and makes no network calls.
 */
...
// Source: stitch.withgoogle.com dashboard. No API endpoint to fetch real usage.
...
function loadQuota(): QuotaState {
	try {
		if (fs.existsSync(QUOTA_FILE)) {
			...
			// Auto-reset if date changed (UTC midnight)
...
function check(): void {
	...
	if (remaining <= 0) {
		console.error("[X] Daily quota exhausted. Use mk:frontend-design as fallback.");
		process.exit(2);
