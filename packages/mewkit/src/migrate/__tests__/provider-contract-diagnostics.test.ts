	...
	it("separates provider support level from surface contract status", () => {
		...
		expect(
			diagnostics.every((diagnostic) =>
				["verified", "experimental", "deprecated"].includes(diagnostic.providerSupportLevel),
			),
		).toBe(true);
		expect(
			diagnostics.every((diagnostic) => ["documented", "partial", "unsupported"].includes(diagnostic.surfaceStatus)),
		).toBe(true);
	});
