...
const impact = (over: Partial<{ scoutRequired: boolean; sourceChanged: number; totalChanged: number; changedFiles: ChangedFile[] }> = {}) => ({
...
describe("selectTier (boundaries)", () => {
	...
	it("medium just above small, below large", () => {
		expect(selectTier({ sourceChanged: THRESHOLDS.SMALL_SRC + 1, totalChanged: THRESHOLDS.SMALL_TOTAL + 1 })).toBe("medium");
	});
...
describe("buildRoster topology", () => {
	...
	it("medium tier adds edge-failure + maintainability-performance", () => {
		...
		expect(r.entries.map((e) => e.dimension)).toEqual(expect.arrayContaining(["edge-failure", "maintainability-performance"]));
	});

	it("large tier has whole-diff roles + per-chunk territory reviewers", () => {
		const changedFiles: ChangedFile[] = Array.from({ length: 9 }, (_, i) => ({ path: `src/f${i}.ts`, kind: "source", deleted: false, added: 5, removed: 1 }));
		const r = buildRoster(impact({ sourceChanged: THRESHOLDS.LARGE_SRC + 1, changedFiles }));
		expect(r.tier).toBe("large");
		expect(r.entries.filter((e) => e.wholeDiff).map((e) => e.id)).toEqual(expect.arrayContaining(["issue-fidelity", "removed-behavior", "cross-file-tracer", "test-matrix"]));
	...
	it("caps territory reviewers at MAX_TERRITORIES", () => {
		const changedFiles: ChangedFile[] = Array.from({ length: 100 }, (_, i) => ({ path: `src/f${i}.ts`, kind: "source", deleted: false, added: 2, removed: 0 }));
		const r = buildRoster(impact({ sourceChanged: THRESHOLDS.LARGE_SRC + 1, changedFiles }));
	...
	it("adds three invariant slices per heavily-rewritten file", () => {
		const heavy: ChangedFile = { path: "src/big.ts", kind: "source", deleted: false, added: THRESHOLDS.HEAVY_FILE_MIN, removed: THRESHOLDS.HEAVY_FILE_MIN };
		...
		expect(inv.map((e) => e.dimension)).toEqual(["invariant:lifecycle-state", "invariant:error-retry", "invariant:config-early-return"]);
	});
...
describe("renderBrief", () => {
	it("states the wrapper as MANDATORY with the exact command and the role focus", () => {
		const r = buildRoster(impact({ sourceChanged: THRESHOLDS.LARGE_SRC + 1, changedFiles: [{ path: "src/a.ts", kind: "source", deleted: false, added: 3, removed: 1 }] }));
