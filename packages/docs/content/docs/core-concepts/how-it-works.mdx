...
| L3: Cook | `/mk:cook` workflow | Drives the task through [the seven phases](/core-concepts/workflow) and their two gates |
...
| L6: Skills | The skill library | Just-in-time activation, loaded only when the task domain matches |
...
| Store | Schema-validated JSON in `.meowkit/memory/` | Canonical files in `tasks/wikis/<slug>/` + derived FTS index |
...
```mermaid
flowchart TD
    U["User: 'Add Stripe payment checkout'"]
    P0["<b>Phase 0</b> — orchestrator classifies → COMPLEX (fintech domain)<br/>Loads architecture-decisions.md for past payment patterns"]
    P1["<b>Phase 1</b> — planner creates plan<br/>GATE 1: human approves"]
    P2["<b>Phase 2</b> — tester writes failing tests (if --tdd enabled)"]
    P3["<b>Phase 3</b> — developer implements → build-verify.cjs checks every file<br/>budget-tracker.cjs warns at $30, blocks at $100"]
    P4["<b>Phase 4</b> — reviewer audits 5 dimensions<br/>GATE 2: human approves"]
    P5["<b>Phase 5</b> — shipper creates PR, never pushes to main"]
    P6["<b>Phase 6</b> — documenter captures patterns to memory for next time"]

    U --> P0 --> P1 --> P2 --> P3 --> P4 --> P5 --> P6
```
