...
Extracts patterns, decisions, and failures from the session. Writes to the appropriate topic files in `.meowkit/memory/`. Updates documentation. Closes the task.
