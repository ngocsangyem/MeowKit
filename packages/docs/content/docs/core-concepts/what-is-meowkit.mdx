...
With the same model, the same task, and the same compute budget, changing only the environment design raises performance by 64% ([SWE-agent, NeurIPS 2024](https://arxiv.org/abs/2405.15793)). The model is not the bottleneck. The environment is.
...
- **[ClaudeKit](https://claudekit.cc/)** gave this its shape. It was the original structured skill system for Claude Code. Its orchestration protocols, development rules, and cook workflow are what MeowKit's phase-gated pipeline descends from.
- **[Gstack](https://github.com/garrytan/gstack)** is a broad skill collection covering headless browser automation and QA testing, and the source of the "search before building" habit MeowKit inherits.
- **[Aura-frog](https://github.com/nguyenthienthanh/aura-frog)** is an autonomous agent framework whose multi-agent coordination and self-healing patterns shaped MeowKit's agent architecture and escalation protocols.
...
- [How It Works](/core-concepts/how-it-works): architecture architecture walkthrough
