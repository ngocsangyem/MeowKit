...
The CLI walks you through a short set of prompts, and every one after the toolkit picker is skippable. See [Installation](/installation) for the full list and the flags that skip them. Takes ~30 seconds.
...
This runs the complete workflow. Each phase activates the appropriate agent, and you will meet both hard gates along the way. See [Workflow Phases](/core-concepts/workflow) for what each one does.
...
- [Workflow Phases](/core-concepts/workflow): understand Phase 0-6
- [Build a Feature](/guides/build-a-feature): the feature workflow end to end
- [Fix a Bug](/guides/fix-a-bug): the bug workflow end to end
- [Debug Effectively](/guides/debug-effectively): structured debugging guide
