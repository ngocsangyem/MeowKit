...
```bash
/mk:intake PROJ-123         # fetch
/mk:jira-evaluator PROJ-123 # complexity, missing acceptance criteria, contradictions
/mk:jira-estimator PROJ-123 # Fibonacci story points, with reasoning
```

**You end up with** an assessment you can act on. Nothing is written to Jira; every suggested
change waits for you.
**The machinery** is three read-only intelligence leaves, listed on [Jira Integration](/guides/jira-integration).
...
- [Build a Feature](/guides/build-a-feature) — implementation after evaluation
