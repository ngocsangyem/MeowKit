...
**You end up with** the media turned into text you can reason about, and a comment on the
ticket recording it.
**Worth knowing** the fallback chain degrades rather than fails: a missing tool means a
lower-fidelity read, not an error.
...
```mermaid
flowchart LR
    T["Ticket arrives<br/>(Jira / Linear / GitHub / manual)"]
    T --> A["Text content"] --> A1["mk:intake analyzes directly"]
    T --> B["Images (.png/.jpg)"] --> B1["Claude Read or Gemini"]
    T --> C["Videos (.mp4/.mov)"] --> C1["FFmpeg key frames"] --> C2["Claude Read or Gemini"]
    T --> D["Figma links"] --> D1["Figma MCP or PNG export"] --> D2["Claude Read or Gemini"]
    T --> E["PDFs"] --> E1["Claude Read<br/>(native, up to 20 pages)"]
```
...
| Gemini API key | Better image/video analysis | Set `MEOWKIT_GEMINI_API_KEY` in `.meowkit/.env` |
...
<Callout title="None of these are required" type="info">
...
flowchart TD
    Q{"FFmpeg installed?"}
    Y["Extract key frames (scene changes only)<br/><code>ffmpeg -i bug-reproduction.mp4 -vf select='gt(scene,0.3)' -vsync vfq frame_%03d.jpg</code>"]
    Y2["Result: 8-15 key frames from a 2-minute video<br/>3600 raw frames → 12 key frames = 99.7% reduction"]
    N["Cannot analyze video"]
    N2["'Video attached but FFmpeg not installed.<br/>Install: brew install ffmpeg — or — npx mewkit setup --system-deps<br/>Workaround: manually screenshot key moments and attach as images'"]
...
flowchart TD
    Q{"Gemini available?"}
    Y["Batch analyze all frames"]
    Y2["frame_001: 'Login page, empty form'<br/>frame_003: 'Email entered, password typed'<br/>frame_005: 'Submit clicked, loading spinner'<br/>frame_008: 'Error toast: Session expired'<br/>frame_012: 'Redirected back to login page'"]
    N["Claude Read on each frame"]
    N2["Similar descriptions,<br/>less precise on small text"]
...
<Callout title="FFmpeg is required for video" type="warn">
...
```mermaid
flowchart TD
    Q{"Figma MCP available?"}
    A["<b>Best</b> — mk:figma ANALYZE mode"]
    A2["get_design_context: component tree, styles, layout, constraints<br/>get_screenshot: visual reference<br/>Extract: exact colors (#1A73E8), exact spacing (24px), fonts (Inter 14px/20px)<br/>Components: LoginCard, EmailInput, PasswordInput, SubmitButton, SocialLoginGroup"]
    B["<b>Fallback</b> — user exports PNG"]
    B2["mk:intake asks: 'Export Figma frames as PNG'<br/>User drops login-screen.png<br/>Claude Read or Gemini analyzes the image<br/>Gets: approximate colors (~#1a73e8), estimated spacing (~24px), visual layout"]
    C["<b>Blocked</b> — cannot analyze design"]
    C2["'Figma link detected but cannot access.<br/>Install Figma MCP: claude mcp add figma<br/>Or export frames as PNG and attach to ticket'"]

    Q -->|yes| A --> A2
    Q -->|"no, but PNG export possible"| B --> B2
    Q -->|"no MCP, no export"| C --> C2
```
...
- [Fix a Bug](/guides/fix-a-bug) — Bug investigation after intake
- [Build a Feature](/guides/build-a-feature) — Feature implementation from Figma spec
