...
**You end up with** a sprint plan backed by codebase evidence rather than by estimates alone.
**The machinery** is Jira reads plus a codebase scout; see [Jira Integration](/guides/jira-integration).
...
3. Optional pre-ticket feasibility scan with `/mk:scout` + `/mk:docs-finder` (see the [Spec → PR walkthrough](/workflows/spec-to-pr-walkthrough/step-03-tech-feasibility) for the full pattern)
