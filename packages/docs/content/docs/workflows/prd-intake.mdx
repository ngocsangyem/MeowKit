...
```bash
/mk:intake PRD-123          # classify area, score completeness, run RCA if it is a bug
/mk:jira-evaluator PRD-123  # readiness
/mk:jira-lifecycle transition PRD-123 "In Analysis"
```

**You end up with** a structured analysis posted back to the ticket as a comment.
**Worth knowing** the analysis skills work on pasted text too, so you can try this before
setting up Jira access at all.
...
<Callout title="Simplified with mk:intake (v2.0)" type="info">
...
<Callout title="Evaluator vs completeness scoring" type="info">
...
<Callout title="Execute the suggested actions" type="info">
...
```mermaid
flowchart LR
    I["<b>mk:intake</b><br/>orchestrator"]
    I --> A["mk:jira-issue — fetch ticket"]
    I --> B["mk:scale-routing — product-area classification"]
    I --> C["mk:plan-creator — completeness scoring (scope challenge)"]
    I --> D["mk:scout — codebase context"]
    I --> E["mk:investigate — RCA (bug tickets)"]
    I --> F["mk:decision-framework — triage rules"]
    I --> G["mk:jira-collaborate — post analysis comment"]
    I --> H["mk:jira-lifecycle — transition + assign (Step 7)"]
    I --> J["mk:jira-relationships — link related tickets (Step 7)"]
```
...
- **No `MEOW_JIRA_*` env = no Jira I/O**: The analysis skills work standalone (you can paste ticket text), but automated read/write requires the three env vars in `.meowkit/.env`.
...
- [Build a Feature](/guides/build-a-feature) — Full implementation workflow after intake
- [Fix a Bug](/guides/fix-a-bug) — Bug-specific investigation workflow
- [Review Code](/guides/review-code) — Review workflow for the fix
