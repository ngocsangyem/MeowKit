...
- Density detection is automatic when the host sends a model at SessionStart. Set `MEOWKIT_MODEL_HINT` only as a fallback or to force a tier — see [Adaptive Density](/guide/adaptive-density)
...
That's it. The harness detects your model, selects density, and runs the full pipeline.
...
The harness classifies the detected model into a tier:
...
Detection reads the `model` field from SessionStart stdin. If your Claude Code build does not send that field, `model-detector.cjs` reports "not detected" and falls back to `MEOWKIT_MODEL_HINT`. Set `export MEOWKIT_MODEL_HINT=opus-4-6` in your shell, restart Claude Code, and the tier resolves from the hint.
