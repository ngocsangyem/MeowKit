...
```bash
/mk:jira-analyst AUTH-201                       # read the ticket and its attachments
/mk:jira-evaluator AUTH-201                     # is it ready to work on?
/mk:jira-lifecycle transition AUTH-201 "In Progress"
/mk:cook AUTH-201                               # plan, build, review
/mk:ship                                        # commit, PR, CI
```

**You end up with** a merged-ready PR and the ticket moved to its next status.
**The machinery** is the seven-phase pipeline, explained once on [Workflow Phases](/core-concepts/workflow).
...
- [Build a Feature](/guides/build-a-feature) — general feature workflow
- [Fix a Bug](/guides/fix-a-bug) — bug-specific investigation
- [Review Code](/guides/review-code) — review workflow details
- [Ship Safely](/guides/ship-safely) — deploy pipeline details
