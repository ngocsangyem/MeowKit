...
```bash
/mk:sprint-contract sprint-goal set "<goal>" --sprint 42   # at sprint start
/mk:jira-agile sprint add PROJ-125 --sprint 42             # during
/mk:jira-agile sprint close --sprint 42                    # at sprint end
/mk:retro                                                  # after
```

**You end up with** sprint governance that loads only when an Agile context is detected;
non-Agile sessions pay nothing for it.
**The machinery** is three conditional rule files, not a skill you invoke.
...
<Callout title="Harness incompatibility" type="warn">
