...
- Node.js 24+, Python 3.9+, Git installed
...
**What gets created:** A `.claude/` directory with 41 agents, 126 skills, lifecycle hooks, security rules, and a memory system. Plus `CLAUDE.md` (the entry point Claude reads at session start).
...
Claude reads `CLAUDE.md` automatically. The **orchestrator** agent activates and reads `memory/cost-log.json` for cost history. Topic files in `.meowkit/memory/` (`fixes.md`, `review-patterns.md`, `architecture-decisions.md`) are loaded on-demand by consumer skills at task start — they are empty on first run.
...
→ [Build a Feature](/guides/build-a-feature) — how to add features after initial setup
