...
## Unreleased — Runtime-Neutral State Root & Codex-First Migration

Project state moves out of the provider directory into a runtime-neutral `.meowkit/` root, `mewkit migrate` safely relocates any legacy `.claude/memory/` content, and the migration surface is trimmed to the three actively-maintained targets.
...
- Removed `mewkit build-plugin` and `mewkit validate --plugin` / `--parity`.
