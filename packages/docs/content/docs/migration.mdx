...
✓ converted from your `.claude/` kit · bundle = shipped by the authored Codex bundle, your own files are not converted · — not supported by tool

**Codex does not convert your own agents, commands, or hooks.** The converter leaves those three surfaces alone and prints a one-line notice when it finds them. Toolkit agents, commands, rules, modes, and hooks reach Codex through a hand-authored Codex-native bundle instead. Port your custom ones by hand.

**Special cases:**
- **Toolkit commands** ship as Agent Skills in the bundle (`.agents/skills/*/SKILL.md`) — Codex custom prompts gave way to skills. Your own `.claude/commands/` files are not converted; dynamic template syntax (`$ARGUMENTS`, `$1`, <code v-pre>{`{...}`}</code>, `` !`cmd` ``, `@file`) has no skill equivalent, so port those manually.
...
- **`.meowkit/.env`** converts to a Codex `[shell_environment_policy]` scaffold from key names only. Values are never copied; secret-like key names are omitted and counted only as an aggregate warning.
- **Hooks** reach Codex and Cursor through their authored bundles only; the legacy `.claude`→`.cursor` converter emits none, so `mewkit migrate cursor` skips hooks with a warning. Codex events are version-gated: recent Codex (0.142+) supports 10 events with hooks enabled by default; older versions fall back to the conservative 6-event table. Cursor's own lifecycle events are documented and shipped in the Cursor bundle's `.cursor/hooks.json`.
...
| Unmapped runtime (`.claude/scripts/`, `.meowkit/memory/`) | neutralized to provider-agnostic phrasing | preserved + warned | preserved |
...
- [Portability commands](/cli/portability#migrate) — every flag with examples
- [Memory System](/guide/memory-system#legacy-migration-from-claudememory) — every `mewkit migrate` run also migrates legacy `.meowkit/memory/` into `.meowkit/`
