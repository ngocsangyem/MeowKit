...
| `post-session.sh`      | Stop        | Session end | Capture session data to `.meowkit/memory/`                                                 | No           |
...
| `append-trace.sh`   | Any       | Append JSONL trace record to `.meowkit/telemetry/trace-log.jsonl` (secret-scrubbed, atomic, auto-rotates at 50MB) | No                       |
...
The `MEOWKIT_HOOK_PROFILE` environment variable controls which hooks are active. Set it in your `.env` file or shell before starting a Claude Code session.
...
<Callout title="Safety-critical hooks never skip" type="warn">
...
| `MEOWKIT_HOOK_PROFILE=fast`         | Skip `pre-ship`, `post-session`, `learning-observer` (speed)          |
...
|---|---|---|
| `PostToolUse` | Dispatch | Advisory |
| `PostToolUse` | Learning observer | Advisory |
| `PostToolUse` | Security scanning written file | **Blocking** |
| `PostToolUseFailure` | Posttoolfailure probe | Advisory |
| `PreCompact` | Precompact probe | Advisory |
| `PreToolUse` | Checking file access permissions | **Blocking** |
| `PreToolUse` | Checking Gate 1 approval | **Blocking** |
| `PreToolUse` | Checking web fetch targets | **Blocking** |
| `PreToolUse` | Pre task check | Advisory |
| `PreToolUse` | Running pre-ship validation | **Blocking** |
| `SessionStart` | Dispatch | Advisory |
| `SessionStart` | Ensuring Python skills venv | Advisory |
| `SessionStart` | Loading capability resolution | Advisory |
| `SessionStart` | Loading project context | Advisory |
| `SessionStart` | Validating Jira env | Advisory |
| `Stop` | Control probe | Advisory |
| `Stop` | Dispatch | Advisory |
| `Stop` | Post session | Advisory |
| `Stop` | Pre completion check | **Blocking** |
| `UserPromptSubmit` | Detecting --tdd flag | Advisory |
| `UserPromptSubmit` | Dispatch | Advisory |

Registered with no hooks today: `SubagentStart`, `SubagentStop`.
