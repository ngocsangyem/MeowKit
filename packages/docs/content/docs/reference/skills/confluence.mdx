...
| | |
|---|---|
| **Source** | [`.claude/skills/confluence/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/confluence/SKILL.md) |
| **Owner** | `confluence` |
| **Runtime** | `claude-code` |
| **Risk** | medium |
| **Phase** | `on-demand` |
...
Install, authentication, the Cloud-only boundary, and troubleshooting live on [Confluence Integration](/guides/confluence-integration).

Mechanically: the wrapper exits 3 on non-`*.atlassian.net` URLs, and the hub does NOT auto-fallback to the Atlassian MCP transport - the leaf is invoked with it explicitly.chitecture
