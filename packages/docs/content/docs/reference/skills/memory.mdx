---
title: "mk:memory"
description: "Session memory toolkit — capture learnings, extract patterns, track costs, and consolidate stale entries. Persists to .meowkit/memory/ topic files. Activates during Phase 0 (Orient) to load context and Phase 6 (Reflect) to persist it."
---
...
| | |
|---|---|
| **Source** | [`.claude/skills/memory/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/memory/SKILL.md) |
| **Owner** | `memory` |
| **Runtime** | `claude-code` |
| **Risk** | high |
...
Topic-file-scoped session memory. Captures session learnings, extracts patterns for CLAUDE.md promotion, tracks token usage and costs, and consolidates stale entries. Persists to `.meowkit/memory/` topic files. Activates during Phase 0 (Orient) to load context and Phase 6 (Reflect) to persist it. NOT for weekly-cadence engineering retrospectives (see `mk:retro`); NOT for arbitrary file storage.
