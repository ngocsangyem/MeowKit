...
The SessionStart hook `jira-env-loader.sh` validates `.meowkit/.env` and emits `[mk:jira] env OK` / `[mk:jira] <KEY> missing`. The wrapper `scripts/jira-as.sh` translates `MEOW_JIRA_*` → native `JIRA_*` env per call.
