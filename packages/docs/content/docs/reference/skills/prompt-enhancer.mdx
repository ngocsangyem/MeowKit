...
| | |
|---|---|
| **Source** | [`.claude/skills/prompt-enhancer/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/prompt-enhancer/SKILL.md) |
| **Owner** | `docs` |
| **Runtime** | `claude-code` |
| **Risk** | medium |
| **Phase** | `on-demand` |
...
4. **Scout** *(when `--deep`)* — invoke `mk:scout` with allow-list filter; ≤8 files / ≤100 lines/file / ≤30s wall clock; default-deny against `.meowkit/memory/*`, `.env*`, `tasks/`, secrets.
...
| Forbid-list (default-deny) | `.meowkit/memory/*`, `.env*`, `tasks/`, `~/.ssh/`, `*.pem`, `*.key`, `*credentials*`, `*secret*`, anything matched by `.gitignore` |
