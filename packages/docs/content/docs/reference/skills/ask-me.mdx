...
| | |
|---|---|
| **Source** | [`.claude/skills/ask-me/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/ask-me/SKILL.md) |
| **Owner** | `research` |
| **Runtime** | `portable` |
| **Risk** | medium |
...
| API contract design or review | `mk:api-design-principles` |
| Backend change end to end | `mk:backend-development` |
| DB schema / migration / query design | `mk:database` |
| Infrastructure, CI/CD, deployment safety | `mk:devops` |
...
- **Forbidden paths are never read:** `.env*`, secrets/keys/credentials, `.meowkit/memory/`, `session-state/`, dependency and build directories.
