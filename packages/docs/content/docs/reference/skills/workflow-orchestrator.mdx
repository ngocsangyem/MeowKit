...
| | |
|---|---|
| **Source** | [`.claude/skills/workflow-orchestrator/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/workflow-orchestrator/SKILL.md) |
| **Owner** | `lifecycle` |
| **Runtime** | `claude-code` |
| **Risk** | high |
...
The central workflow coordinator. Auto-invoked when a session starts with complex-feature intent. Routes through [the seven phases](/core-concepts/workflow), managing gate enforcement, token budgets, agent team composition, and state persistence. Defers to `mk:cook` for single-task invocations.
...
- **Phase routing:** each phase dispatched to its owning skill — agent-detector and scale-routing at Orient, plan-creator at Plan, testing at Test, developer plus the mandatory simplify and verify stages at Build, review, ship, and memory after. The phase sequence itself is on [Workflow Phases](/core-concepts/workflow).
