...
| | |
|---|---|
| **Source** | [`.claude/skills/agent-detector/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/agent-detector/SKILL.md) |
| **Owner** | `lifecycle` |
| **Runtime** | `claude-code` |
| **Risk** | medium |
...
4. **Step 2: Check Project Context** — Read `.meowkit/memory/fixes.md`, `review-patterns.md`, `architecture-decisions.md`. If insufficient, run `mk:scout` or read `package.json`/`tsconfig.json` directly.
...
8. **After Detection** — Load agent instructions from `.claude/agents/[agent-name].md`. Invoke the appropriate skill (mk:cook, mk:fix, mk:investigate, mk:review, mk:cso). Context loaded on demand from `.meowkit/memory/` topic files.
