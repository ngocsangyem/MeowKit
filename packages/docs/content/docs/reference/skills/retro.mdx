...
| | |
|---|---|
| **Source** | [`.claude/skills/retro/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/retro/SKILL.md) |
| **Owner** | `docs` |
| **Runtime** | `claude-code` |
| **Risk** | medium |
...
8. Review history from `.meowkit/telemetry/reviews.jsonl`
9. TODOS.md backlog
10. Total test file count
11. Regression test commits in window
12. Skill usage telemetry from `.meowkit/telemetry/skill-usage.jsonl`
...
9. Save snapshot to `.meowkit/memory/retros/global-{date}-{seq}.json`
...
- Saves JSON snapshot to `.meowkit/memory/retros/` with sequence numbering
...
4. **Output** -- narrative with tweetable summary, save JSON snapshot to `.meowkit/memory/retros/`
...
| Document as no-action | Append to `.meowkit/memory/retros/{date}-decisions.md` with reason |
