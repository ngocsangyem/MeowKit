...
| | |
|---|---|
| **Source** | [`.claude/skills/plan-creator/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/plan-creator/SKILL.md) |
| **Owner** | `lifecycle` |
| **Runtime** | `claude-code` |
| **Risk** | critical |
...
| `--advice` | Opt-in strategic supervision for one planning run by the [athena](/reference/agents/athena) agent: direction after the scope challenge, rescue on an unresolved red-team contradiction, an independent read before the gate. Up to 4 calls; it never writes the plan and never approves Gate 1 |
