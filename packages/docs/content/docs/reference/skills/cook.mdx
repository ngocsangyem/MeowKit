...
| | |
|---|---|
| **Source** | [`.claude/skills/cook/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/cook/SKILL.md) |
| **Owner** | `lifecycle` |
| **Runtime** | `claude-code` |
| **Risk** | critical |
...
- **Full pipeline:** drives all seven phases, including the mandatory Simplify and Verify stages inside Build — see [Workflow Phases](/core-concepts/workflow)
...
| `--advice` | (modifier) — opt-in strategic supervision by the [athena](/reference/agents/athena) agent: direction after Gate 1, rescue on a stall, an independent read after Verify and before the reviewer, and a correction it can send back. Up to 5 calls; approves nothing |
...
- Forgetting memory read/write — Phase 0 reads `.meowkit/memory/`, Phase 6 appends
