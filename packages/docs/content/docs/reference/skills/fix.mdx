...
| | |
|---|---|
| **Source** | [`.claude/skills/fix/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/fix/SKILL.md) |
| **Owner** | `lifecycle` |
| **Runtime** | `claude-code` |
| **Risk** | high |
...
- **Fix memory:** Reads `.meowkit/memory/fixes.md` and `fixes.json` for prior patterns — repeated bugs become instant fixes
...
| `--advice` | Opt-in advisory supervision. Asks the [athena](/reference/agents/athena) agent for one counsel packet at up to three declared checkpoints — two failed approaches, an irreversible step, unclear residual risk — records the disposition, and continues. Standard and deep profiles only; counsel clears no gate and counts as no verification. Experimental: delegation is not yet confirmed on any runtime, and an unreachable agent prints an unavailable-checkpoint line and continues unsupervised. |
...
Read `.meowkit/memory/fixes.md` and `fixes.json` for prior patterns. Search for similar symptoms, error messages, or affected modules. If a matching fix pattern exists → use as starting hypothesis. If a matching success pattern exists → apply known fix approach directly. This turns repeated bugs into instant fixes.
