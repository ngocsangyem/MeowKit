...
| | |
|---|---|
| **Source** | [`.claude/skills/ship/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/ship/SKILL.md) |
| **Owner** | `lifecycle` |
| **Runtime** | `claude-code` |
| **Risk** | high |
...
/mk:ship release --advice  # Strategic supervision for this stage
```

`--advice` is opt-in strategic supervision by the [athena](/reference/agents/athena) agent: direction once the scope is resolved and pre-flight has run, rescue on an exceptional blocker, and a read of the outcome after CI reaches a terminal green state. The budget is **4 calls per release stage** — `prepare`, `release` and `publish` each carry their own. Counsel is never authorization: it creates no authority to push, open a PR, merge, version, publish, or deploy, and red or pending CI keeps its existing repair-or-stop route.
...
- Reads memory: `.meowkit/memory/architecture-decisions.md` (release context only)
