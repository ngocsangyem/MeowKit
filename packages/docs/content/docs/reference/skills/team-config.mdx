...
| | |
|---|---|
| **Source** | [`.claude/skills/team-config/SKILL.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/skills/team-config/SKILL.md) |
| **Owner** | `git` |
| **Runtime** | `claude-code` |
| **Risk** | medium |
...
```mermaid
flowchart TD
    A["Plan approved (Gate 1)"]
    B["Orchestrator identifies parallel opportunity"]
    C["<b>mk:team-config</b>"]
    C1["1. Parse plan for independent subtasks"]
    C2["2. Generate ownership map"]
    C3["3. Validate zero overlap between ownerships"]
    C4["4. Create git worktrees via mk:worktree"]
    C5["5. Create task-queue.json via mk:task-queue"]
    D["Parallel agents start claiming tasks"]

    A --> B --> C --> C1 --> C2 --> C3 --> C4 --> C5 --> D
```
