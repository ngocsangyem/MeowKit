...
| | |
|---|---|
| **Source** | [`.claude/agents/orchestrator.md`](https://github.com/ngocsangyem/MeowKit/blob/main/.claude/agents/orchestrator.md) |
| **Owner** | `lifecycle` |
| **Runtime** | `claude-code` |
| **Risk** | critical |
| **Writes** | `.claude/session-state/tdd-mode`, `/`, `rules/scale-adaptive-rules.md` |
