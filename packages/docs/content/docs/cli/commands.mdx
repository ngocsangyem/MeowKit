...
Create and manage structured task files. See [Planning and Tasks](/cli/planning) for details.
...
- Hooks reach Codex only through the bundle, which ships `.cjs` handlers registered in `.codex/hooks.json`. Cursor has no documented hooks surface at all.
