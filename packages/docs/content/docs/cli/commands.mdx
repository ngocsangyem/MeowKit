...
`mewkit init` accepts `--target` so you can scaffold a provider toolkit directly, plus an interactive picker.

```bash
# Codex-only toolkit — copies the authored Codex bundle (no .claude/)
npx mewkit init --target codex

# Cursor toolkit — unpacks .claude/ then exports to Cursor
npx mewkit init --target cursor

# Interactive picker after the kit unpacks
...
| Flag                  | Description                                                                                                                                        |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--target <provider>` | Create a provider toolkit. `codex` copies the authored Codex bundle (codex-only, no `.claude/`); any other provider (e.g. `cursor`) unpacks `.claude/` then exports to it |
