...
| `--reason` `<value>` | Why — required even when the directive was adopted |
...
| `--evidence` `<value>` | Path to the workflow evidence index a correction supersedes, or the packet to validate |
| `--correction-kind` `<value>` | source (keeps Gate 1) or scope (invalidates it); input\|output for validate-packet |
