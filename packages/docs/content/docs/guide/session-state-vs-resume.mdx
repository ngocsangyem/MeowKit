...
<Callout title="Reframe" type="info">
...
| "Did this session go over budget?" | `session-state/budget-state.json` (live) + `.meowkit/telemetry/cost-log.json` (historical) |
...
| "What was the durable lesson from the last bug fix?" | `.meowkit/memory/fixes.md` (loaded on demand by `mk:fix`) |
...
- [Memory System](/guide/memory-system) — `.meowkit/memory/` topic files (durable, human-curated learnings) — distinct from session-state
- [Workflow Phases](/core-concepts/workflow) — where each phase reads/writes session-state
