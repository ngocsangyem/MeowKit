...
flowchart TD
    ...
    B --> C[.meowkit/telemetry/trace-log.jsonl]
...
**Location:** `.meowkit/telemetry/trace-log.jsonl`
...
```
.meowkit/telemetry/trace-log.jsonl              ← active log (reset to empty after rotation)
.meowkit/telemetry/trace-log.{YYMMDD-HHMMSS}.jsonl.gz  ← compressed archive
```
...
- `.meowkit/telemetry/trace-log.jsonl` — append-only trace store
