...
provider-neutral tier. This is different from the Claude Code adapter (which picks
a session model by task complexity — see [Model Routing](/docs/guide/model-routing))
and from Codex (which writes a portable reasoning-effort ladder). See
[Model Routing](/guide/model-routing) for the Claude Code and Codex adapters.
Cursor exposes a real model picker per agent, so the bundle assigns each agent the
model that fits its role.
...
<Callout type="info">
  The bundle installs a small **core** pack by default (`explorer`, `planner`,
