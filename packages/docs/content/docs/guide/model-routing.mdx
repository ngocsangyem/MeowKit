...
| Provider | Where | How |
|---|---|---|
| Claude Code | `.claude/agents/<name>.md` frontmatter | `model:` — a tier alias (`opus` / `sonnet` / `haiku`), a named profile pinning an exact id (`fable`, `claude-opus-5`), or `inherit`. See [Claude Agent Models](/guide/claude-agent-models) for the full per-agent table. |
...
All three targets pin a model per agent rather than deriving one from the session tier: each agent gets the model its role actually needs, so bounded wrapper work does not run at top-tier cost. See [Claude Agent Models](/guide/claude-agent-models), [Codex Agent Models](/guide/codex-agent-models), and [Cursor Agent Models](/guide/cursor-agent-models) for the per-agent tables and for how to override a pin.
...
- [Claude Agent Models](/guide/claude-agent-models) — per-agent pins in the Claude Code toolkit
