...
| Codex | `.codex/agents/<name>.toml` | `model` + `model_reasoning_effort` — a concrete Codex model id (e.g. `gpt-5.6`, `gpt-5.6-terra`, `gpt-5.4-mini`) with its own effort (`low`/`medium`/`high`). See [Codex Agent Models](/guide/codex-agent-models) for the full per-agent table. |
