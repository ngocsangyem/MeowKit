...
The Codex value is derived from `model:` at migration time (`.claude/agents/*.md` is the single source), so the two providers stay in sync. Codex model **ids** are deployment-specific and are never hardcoded — only the portable reasoning-effort ladder is written.
