...
| Codex | `.codex/agents/<name>.toml` | `model_reasoning_effort`, derived from the agent's Claude `model:` tier: opus/fable → `xhigh`, sonnet → `high`, haiku → `medium`; `inherit` omits it and inherits Codex's default |
...
The Codex value is derived from `model:` at migration time (`.claude/agents/*.md` is the single source), so the two providers stay in sync. Codex model **ids** are deployment-specific and are never hardcoded — only the portable reasoning-effort ladder is written. The Cursor native bundle instead pins concrete Cursor model ids per agent — see [Cursor Agent Models](/guide/cursor-agent-models).
