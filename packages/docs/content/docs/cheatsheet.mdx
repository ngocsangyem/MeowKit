...
Source of truth: `.meowkit/.env.example`. Most defaults live in `.claude/settings.json` `env` block (team-shared, git-committed); override here per-project / per-user. Uncommented values in `.meowkit/.env` win over `settings.json`; shell exports win over both.
...
```
shell export  >  .meowkit/.env  >  .claude/settings.json "env" block
```
