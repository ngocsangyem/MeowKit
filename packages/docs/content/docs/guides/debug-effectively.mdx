...
|---------------|-------|-----|
| Bug, error, test failure, or anything broken you want fixed | `/mk:fix` | Top-level orchestrator. Invokes scout + investigate + sequential-thinking internally. |
...
| Stuck on approach: "tried five ways, still feels wrong" | `/mk:problem-solving` | Strategic unsticking. 7 techniques. Not for debugging. |

**Default when unsure:** anything that smells like "something is broken" → `/mk:fix`. It invokes the others internally.

## How they compose

`/mk:fix` is the orchestrator. It calls `/mk:investigate` (collect evidence), which calls `/mk:sequential-thinking` (eliminate hypotheses). `/mk:problem-solving` stands apart: it is for strategic blocks, not bugs.
...
→ `/mk:investigate`. Pure diagnostic, since the deploy already succeeded on retry. Investigate produces a DEBUG REPORT; you decide if a fix is needed.

**Architecture trade-off with competing positions:**
→ `/mk:sequential-thinking`. Not a bug but a reasoning task. Hypothesis table for "pick between known options with evidence."
...
- **Fixing symptoms.** Trace backward to the root cause
- **Skipping scout.** `/mk:fix` Step 1 is mandatory
- **No regression test.** `/mk:fix` Step 5 blocks without one
- **Three or more failed attempts.** Stop and question the architecture
