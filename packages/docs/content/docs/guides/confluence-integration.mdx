...
MeowKit reads and writes Confluence through the `confluence-as` CLI. If you have already set up [Jira](/guides/jira-integration), most of this is done — the same Atlassian API token works for both.
...
Two routing rules are worth knowing because they are not guessable. Labels belong to `mk:confluence-collaborate`, not to `mk:confluence-page` — "label this page" is a collaboration operation. So are comments, attachments, and watchers, even when scoped to a single page.
