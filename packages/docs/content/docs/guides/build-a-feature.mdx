...
| The developer self-heals three times, then escalates | The tests expect a different approach than the implementation took | Decide which is wrong — the plan or the tests — and fix that one, not the symptom |
