...
MeowKit takes these foundations and adds **enforced discipline** — hard gates that block, opt-in strict TDD, and security scanning baked into every workflow. The goal isn't to replace these projects but to push the idea further: what happens when best practices aren't suggestions but requirements?
...
> AI agents need enforced discipline — hard gates, security scanning, and human approval — to ship production-quality code. Strict red-first TDD is opt-in: turn it on with `--tdd` or `MEOWKIT_TDD=1`. High-risk work passes the planning, review, and security gates either way.
...
MeowKit is an open-source, community-driven project released under the MIT license. It exists to push the boundaries of what disciplined AI coding agents can do, and to share those learnings freely.
...
- [What Is MeowKit](/core-concepts/what-is-meowkit) — deeper dive into MeowKit's principles
