...
│   └── bin/         Shell utilities
├── memory/          Cost log, trace log, derived wiki index
├── settings.json    Hook registrations
├── meowkit.config.json  Project configuration
├── .env             Optional API keys (Gemini)
...
Curated memory (fix patterns, review patterns, architecture decisions) lives outside the provider directory, at `.meowkit/memory/`.
...
```bash
# Add to .claude/.env — gitignored, secrets stay local
echo "MEOWKIT_GEMINI_API_KEY=your-key" >> .claude/.env
```
