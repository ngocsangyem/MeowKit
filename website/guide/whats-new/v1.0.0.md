---
title: "v1.0.0 — The Disciplined Velocity Release"
description: "Initial release — scale-adaptive routing, hook enforcement, adversarial review, party mode, step-file architecture, parallel execution."
persona: A
---
...
**Solution:** `/mk:help` scans the project and tells you exactly what to do next.

```bash
/mk:help
# → "You're in Phase 1. Gate 1 is not yet approved.
#    Next step: run /mk:cook tasks/plans/260329-auth.md"
```
...
**Solution:** `mk:debug` enforces a 5-step workflow: reproduce → isolate → root cause → fix → verify. The iron rule: no fix without root cause. Includes a `references/common-root-causes.md` covering the patterns behind ~80% of bugs (null paths, race conditions, API mismatches, wrong conditions).
...
**Solution:** `mk:simplify` runs after Phase 3 (Build GREEN) and before Phase 4 (Review). The iron rule: behavior must not change — every simplification must pass the same tests. Removes dead code, flattens nesting, extracts only when 3+ duplicates exist.

## Team Configuration

**Problem:** Setting up parallel agents requires creating worktrees, ownership maps, and task queues manually.

**Solution:** `mk:team-config` automates the entire parallel setup: analyzes the plan for independent subtasks, generates an ownership map with glob patterns, creates git worktrees, and initializes the task queue. Validates zero overlap before any agent starts.
...
**Solution:** Domain-based routing at Phase 0 via `mk:scale-routing`. A CSV maps keyword signals to complexity levels. The CSV verdict overrides manual classification — no exceptions.
...
```bash
/mk:docs-init   # new project — scan codebase, generate skeleton
/mk:docs-sync   # after shipping a feature — diff-aware update
```
...
**Solution:** `mk:review` now uses a 4-step workflow with 3 parallel reviewers and a triage step.
...
**Solution:** `/mk:party` spawns 2-4 deliberation agents with different analytical lenses. Each reaches an independent conclusion. A synthesis agent reconciles positions into a single recommendation.

```bash
/mk:party "PostgreSQL or MongoDB for the session store?"
```
...
| ---------------------- | ----------------------------------------- | ---------------------------- |
| Navigation Help        | "What do I do next?"                      | Explicit `/mk:help`        |
...
| Adversarial Review     | Single-pass review misses bugs            | Phase 4 via `mk:review`    |
| Anti-Rationalization   | Agents bypass rules via reasoning         | Always, built into rules     |
| Party Mode             | No stress-test for architecture decisions | Explicit `/mk:party`       |
| Step-File Architecture | Token waste + no resumability             | Complex skills (mk:review) |
