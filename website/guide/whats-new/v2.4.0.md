...
- **`meow:project-context init`** — new subcommand writes a TODO-filled skeleton from `templates/skeleton.md` for users starting from scratch.
...
flowchart TD
    ...
    C -->|no| WARN[Emit actionable warning:\nnpx mewkit setup OR\nmeow:project-context init]
    INJ --> TASK[Agent reads task-specific context]
    WARN --> TASK
    TASK --> ROUTE[Route to correct phase]
```

Before this release, missing `project-context.md` caused a silent failure — the loader script exited non-zero with no user-visible message. The fix in `meow:plan-creator/step-02-codebase-analysis.md` now emits an actionable warning pointing to the bootstrap command.
...
## New: meow:project-context init Action

`/meow:project-context init` writes a TODO-filled skeleton from `templates/skeleton.md` to `docs/project-context.md`. Designed for users starting a new project who don't yet have a constitution.
...
- `meow:debug`, `meow:documentation`, `meow:shipping` frontmatter gained correct `deprecation` YAML keys

**Path fix (CF4 residual)**
- `meow:cook/SKILL.md:159` — bare `memory/lessons.md` reference corrected to `.claude/memory/lessons.md`
...
- `meow.md` dispatcher: `meow:plan` corrected to `meow:plan-creator`, `meow:test` corrected to `meow:testing`

**INDEX cleanup**
- SKILLS_INDEX: duplicate `meow:scout` row removed, footer count corrected 67 → 74 unique registered
...
|----------|-------|---------|
| SKILL.md files (Gotchas, gate-rules, deprecation) | 19 | meow:vue, meow:typescript, meow:cook, meow:ship |
...
| Docs | 4 | docs/project-context.md (new), README, CLAUDE.md, meow:plan-creator/step-02 |
...
**Projects without `docs/project-context.md`:** The loader now emits an actionable warning instead of failing silently. Run `/meow:project-context init` to generate a skeleton, or `npx mewkit setup` for the full bootstrap.
