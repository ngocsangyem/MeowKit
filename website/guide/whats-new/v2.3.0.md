...
- Auto-strict trigger (Rule 7): when `mk:scale-routing` returns `level=high`, cook auto-enables `--strict` at Phase 4.5
...
Session budget tracks cumulative estimated cost across all tool calls in a single Claude Code session. Harness budget tracks cost within a single `mk:harness` run, which may span multiple sessions.

## Cook Verification Flags

Three modifier flags added to `mk:cook`. Flags layer on top of the detected mode and fire at Phase 4.5 (between Review and Ship):

```mermaid
flowchart TD
    A["Phase 4: Review\n(Gate 2 approved)"] --> B{"scale-routing\ncheck at Phase 0"}
    B --> |"level=high\nAND no --no-strict"| C["Phase 4.5: mk:evaluate\n(strict — ~$2-5)"]
    B --> |"--strict flag\nexplicit"| C
...
| `--strict` | Full `mk:evaluate` run — drives running build, rubric grading, active verification | Hard FAIL blocks ship | ~$2–5 |
| `--no-strict` | Suppress auto-strict trigger from `mk:scale-routing` | — | — |

**Auto-strict (scale-adaptive Rule 7):** When `mk:scale-routing` returns `level=high` at Phase 0, `strict_flag` is automatically set to `true` at Phase 4.5 — even without `--strict` in input. Phase 4.5 logs the reason: `"scale-routing level=high ({domain}), auto-triggering strict evaluation"`. The `--no-strict` flag is the user escape hatch.
...
`mk:review` step 2b (adversarial persona passes) now loads `prompts/skeptic-anchor.md` before each persona dispatch. The anchor is 22 lines covering:
...
This pattern originates from `mk:evaluate`'s `prompts/skeptic-persona.md` (introduced in v2.2.0) and is now applied to the adversarial review pass as well.

See [evaluator agent](/reference/agents/evaluator) and [mk:review](/reference/skills/review) for the full persona architecture.
...
| `.claude/skills/cook/references/intent-detection.md` | MODIFIED | Modifier flags section: `--verify`, `--strict`, `--no-strict`, auto-strict trigger |
| `.claude/skills/cook/references/workflow-steps.md` | MODIFIED | Phase 4.5 section: browser verify and strict evaluator paths |
| `.claude/skills/review/prompts/skeptic-anchor.md` | NEW | 22-line re-anchor prompt for adversarial persona passes |
| `.claude/skills/review/step-02b-persona-passes.md` | MODIFIED | Load + inject skeptic anchor per persona dispatch |
