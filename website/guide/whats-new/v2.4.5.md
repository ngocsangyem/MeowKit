...
description: "New mk:problem-solving skill for stuck-on-approach moments + three diagnostic framework references for mk:sequential-thinking."
persona: A
---

# v2.4.5 — The Thinking Skills Release

Two thinking skills, one clear boundary. `mk:problem-solving` ships seven non-default techniques for when you're stuck on *approach*. `mk:sequential-thinking` gains three diagnostic-framework references for when you're stuck on *cause*. Both sit on distinct axes and cross-reference each other explicitly.

## Highlights

- **New skill `mk:problem-solving`** — seven strategic-unsticking techniques (simplification cascades, collision-zone thinking, meta-pattern recognition, inversion, scale game, first principles, via negativa). Dispatch table routes by stuck-symptom; explicit boundary reroutes debugging requests to `mk:sequential-thinking`.
- **`mk:sequential-thinking` extended with three diagnostic frameworks** — `five-whys-plus.md`, `scientific-method.md`, `kepner-tregoe.md`. Core workflow unchanged; new references load on demand when a specific methodology fits.
...
|---|---|---|
| **Cause** (why is this broken?) | `mk:sequential-thinking` | "This test is failing and I don't know why" |
| **Approach** (which way do I build this?) | `mk:problem-solving` | "I've implemented this 5 ways and it still feels wrong" |

Both descriptions state the boundary explicitly so LLM triggering stays precise.

## New Skill: `mk:problem-solving`
...
- **Overlaps existing meow skill** — pre-mortem / red-team → `mk:cso`; steel-manning → `mk:party`; debiasing → `mk:review`; socratic → `mk:elicit`
...
| Area | Files |
|---|---|
| New skill | `.claude/skills/problem-solving/SKILL.md` + 9 references (when-stuck, 5 claudekit techniques, first-principles, via-negativa, attribution) |
| Skill extensions | `.claude/skills/sequential-thinking/SKILL.md` (References section expanded) + 3 new refs |
