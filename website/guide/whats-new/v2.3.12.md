...
description: "New meow:pack skill for exporting external repos to AI-friendly files. meow:chom refactored with xia-style mode granularity, speed flags, and sharper boundary rules."
persona: A
---
...
- **`/meow:pack <source>`** — auto-named output at `.claude/packs/{YYYYMMDD-HHMM}-{slug}.{ext}`
...
- **Sequential-thinking escalation** — for features with ≥3 architectural layers or stateful workflows, chom emits handoff text directing the user to `/meow:sequential-thinking`, then return to chom. Never auto-invokes.
- **Boundary Rules (new section):** chom emits handoff text only; does NOT invoke `/meow:plan-creator`, `/meow:brainstorming`, `/meow:cook`, or `/meow:sequential-thinking` mid-flow. This is chom's design choice for phase ownership, not a MeowKit platform rule.
...
```mermaid
flowchart LR
    A[Want to explore<br/>current repo?] -->|Yes| S[meow:scout]
    A -->|No| B[Want to replicate<br/>a feature from elsewhere?]
    B -->|Yes| C[meow:chom]
    B -->|No| D[Want to export code<br/>to external tool/human?]
    D -->|Yes, full code| P1[meow:pack]
    D -->|Yes, API surface only| P2[meow:pack --compress]
```
...
- [meow:pack reference](/reference/skills/pack)
- [meow:chom reference](/reference/skills/chom)
- [meow:scout reference](/reference/skills/scout) — correct tool for inbound analysis
