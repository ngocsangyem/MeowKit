...
- **Standalone subcommands** — `/mk:plan red-team {path}`, `/mk:plan validate {path}`, `/mk:plan archive`
...
`--deep` is `--hard` plus per-phase scouting. After step-03 drafts phase files, each phase gets a targeted `mk:scout` on its Related Code Files directories:
...
|------------|-------|-------------|
| `red-team` | `/mk:plan red-team tasks/plans/260411-auth/` | 4-persona adversarial review; writes `red-team-findings.md` + plan.md summary |
| `validate` | `/mk:plan validate tasks/plans/260411-auth/` | Critical question interview; propagates answers to phase files with Validation Log |
| `archive` | `/mk:plan archive` | Scan completed/cancelled plans; optionally capture learnings; move to `.archive/` or delete |
...
| `mk:cook/references/workflow-steps.md` | MODIFIED | --deep/--tdd passthrough, red-team-findings.md in Phase 4 |
| `mk:review/step-01-gather-context.md` | MODIFIED | red-team-findings.md as supplementary context |
| `mk:help/SKILL.md` | MODIFIED | 3 standalone subcommands listed |
| `mk:harness/step-00-tier-detection.md` | MODIFIED | --deep for FULL density |
| `mk:validate-plan/SKILL.md` | MODIFIED | Deduplication note vs plan-creator validation |
| `mk:plan-ceo-review/SKILL.md` | MODIFIED | red-team-findings.md context loading |
| `mk:plan-ceo-review/SKILL.md` | MODIFIED | red-team-findings.md context loading |
| `mk:trace-analyze/SKILL.md` | MODIFIED | --deep for multi-module findings |
| `mk:bootstrap/references/workflow-full.md` | MODIFIED | --deep note for 5+ dir projects |
