...
- **Phase frontmatter contract** — Phase files start with a YAML frontmatter block (`phase`, `title`, `status`, `priority`, `effort`, `dependencies`). Frontmatter is the machine-readable source of truth; the Overview block (`**Status:** ...`) is a human-readable mirror regenerated by sync-back. `status: pending` at creation; `failed`/`abandoned` are terminal.
- **Formal sync-back algorithm** — Cook's finalize step counts `[x]` in `## Todo List` per phase → derives `pending`/`in_progress`/`completed`, rewrites frontmatter, regenerates the Overview mirror. Idempotent. Terminal states never auto-promoted or overwritten. Status read order mirrors `packages/mewkit/src/orchviz/plan/parse-phase-file.ts` so planner, harness, validator, and orchviz agree.
...
| Flag              | Behavior                                                                                                                                          |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| _(none)_          | Auto-detect from scope — trivial exits, simple → fast, complex → hard                                                                             |
