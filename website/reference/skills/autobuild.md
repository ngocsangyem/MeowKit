...
- **Adaptive density:** Auto-selects MINIMAL / FULL / LEAN scaffolding based on model tier via `mk:scale-routing`, honors `MEOWKIT_AUTOBUILD_MODE` override
...
- **Run reports:** Every run produces a full audit trail at `tasks/autobuild-runs/YYMMDD-{slug}/run.md` including budget trail, per-step artifacts, and final verdict
...
/mk:autobuild --resume 260501-1450-build-kanban          # Resume interrupted run
/mk:autobuild "build payment system" --tdd               # TDD enforcement
```
...
| Step | Action |
|---|---|
| 0 — Tier Detection | `mk:scale-routing` picks MINIMAL / FULL / LEAN; honors `MEOWKIT_AUTOBUILD_MODE` |
...
| 6 — Run Report | Append-only audit trail at `tasks/autobuild-runs/YYMMDD-{slug}/run.md` |
...
Override: `--tier` flag or `MEOWKIT_AUTOBUILD_MODE`. Density never bypasses gates.
...
| Artifact | Location |
|---|---|
| Run report | `tasks/autobuild-runs/YYMMDD-{slug}/run.md` |
