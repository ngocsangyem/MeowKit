...
Transform existing code into agent-consumable surfaces. Produces a **Transformation Spec**: which capabilities to expose, what shape each surface should take, and how CLI + MCP + companion skill share a common core. Hands off to `mk:plan-creator` → `mk:cook` for the actual build.

## What This Skill Does

`mk:henshin` analyzes an existing module, library, or product feature and answers four questions:
...
The output is a Transformation Spec that `mk:plan-creator` turns into a multi-phase implementation plan.
...
- **Boundary-respecting** — outbound transformation only; inbound external-repo work routes to `mk:chom`

## When to Use This

::: tip Use mk:henshin when...
...
::: warning Do NOT use mk:henshin when...

- The source is an **external** repo and you want to port it in → use `/mk:chom`
- You're building an MCP server from scratch with no pre-existing code → use `/mk:skill-creator` + `/mk:plan-creator` from a blank spec
- You just want to `npm publish` without an agent-use story → use `/mk:ship` directly
- You need the actual scaffolder, wrap, test, docs, and publish steps — henshin hands off; execution is owned by `/mk:plan-creator` → `/mk:cook`
...
/mk:henshin [feature-or-module]

# Pick a surface subset
/mk:henshin auth --cli             # CLI only
/mk:henshin billing --mcp          # MCP server only
/mk:henshin reports --both         # default — all three surfaces

# Interaction style
/mk:henshin feature --auto         # auto-approve technical defaults (name/license/ownership still gated)
/mk:henshin feature --ask          # explicit 7-question interview

# Lean mode — skip optional references, keep HARD GATE
/mk:henshin feature --lean
```
...
| 6. Handoff | Hand off to `/mk:plan-creator` with the spec attached; henshin does not invoke downstream skills itself |
...
## 7. Handoff — next command: /mk:plan-creator [spec-path]
```
...
henshin emits a Transformation Spec and a handoff directive. It does **NOT** invoke `/mk:plan-creator`, `/mk:cook`, `/mk:skill-creator`, or `/mk:ship` mid-flow.
...
| Next step | `/mk:plan-creator` → `/mk:cook` | `/mk:plan-creator` → `/mk:cook` |

They move code in **opposite directions** and share no semantic overlap.

## Gotchas

- **Not a builder.** henshin writes a spec. If you want code, chain `/mk:plan-creator` + `/mk:cook` after handoff.
...
- **External repo? Wrong tool.** Paste a GitHub URL and henshin will redirect you to `/mk:chom`.
...
- [`mk:chom`](/reference/skills/chom) — inbound counterpart (external → local)
- [`mk:plan-creator`](/reference/skills/plan-creator) — downstream: turns Spec into phased plan
- [`mk:cook`](/reference/skills/cook) — downstream: executes the plan
- [`mk:skill-creator`](/reference/skills/skill-creator) — for building the companion skill from the spec
- [`mk:ship`](/reference/skills/ship) — downstream: publishes the npm package
