...
Step-file workflow that drives a running build behaviorally, probes each rubric criterion via active verification, and produces a graded verdict with runtime evidence. Distinct from [`mk:review`](/reference/skills/review) (structural code audit). Owned by the `evaluator` agent with isolated context — self-evaluation is forbidden.

## What This Skill Does

`mk:evaluate` takes a running build — a URL, a file path, or a running-app handle — and grades it against a composed rubric preset. The evaluator does not read source code; it clicks buttons, sends HTTP requests, and runs CLI commands to verify each acceptance criterion against the live artifact. Every verdict must include non-empty evidence (screenshot, HTTP response capture, or CLI transcript). PASS verdicts without evidence are rejected by `validate-verdict.sh` and converted to FAIL. The skeptic persona is reloaded before every criterion to prevent leniency drift.

## What This Skill Does NOT Do

`mk:evaluate` is not `mk:review`. The difference matters:

| Dimension | `mk:evaluate` | `mk:review` |
|---|---|---|
...
- **Rubric composition** — composes the active preset via `mk:rubric`; frontend-app default is pruned to 4 rubrics (product-depth, functionality, design-quality, originality)
...
::: warning Don't use mk:evaluate when...
- The build has no runnable artifact (pure library, type-only package) — nothing to actively probe
- You want structural code review — use [`mk:review`](/reference/skills/review) for that
- The task is `/mk:fix` simple — overhead exceeds value for tiny changes
...
/mk:evaluate http://localhost:3000

# Evaluate with an explicit rubric preset
/mk:evaluate http://localhost:3000 --rubric-preset fullstack-product

# Evaluate a CLI tool by path
/mk:evaluate ./dist/my-cli --rubric-preset cli-tool

# Cap criteria per session (useful for large presets)
/mk:evaluate http://localhost:3000 --max-criteria 10
...
Calls `mk:rubric compose <preset>` to assemble grading criteria. Loads `prompts/skeptic-persona.md`. Parses the signed sprint contract if one exists for the task (gates on `contract_schema_version:` marker — unversioned contracts are ignored until Phase 4 ships).
...
For each FAIL or WARN criterion, produces a one-line fix suggestion scoped to the active rubric. Emits a handoff message for the harness iteration loop (consumed by step-05-iterate-or-ship in `mk:harness`).
...
1. **Don't grade source code** — if you're reading `.tsx` files instead of clicking buttons, you've drifted into static review; that's `mk:review`'s job
2. **Don't trust test-pass claims** — tests can pass against mocks while the real endpoint returns 500; run the build yourself
3. **Don't auto-load all 7 rubrics** — the pruned 4-rubric frontend preset avoids duplicating work that `mk:review` and `security-rules.md` already cover
...
- [`mk:harness`](/reference/skills/harness) — step 4 of the harness pipeline; invokes `mk:evaluate` after each generator iteration
- [`mk:rubric`](/reference/skills/rubric) — provides the composed rubric presets used for grading
- [`mk:sprint-contract`](/reference/skills/sprint-contract) — signed contract is loaded in step-01 to scope grading to negotiated criteria
- [`mk:review`](/reference/skills/review) — structural code audit; distinct from and complementary to behavioral evaluation
- [`mk:trace-analyze`](/reference/skills/trace-analyze) — consumes evaluator verdict records from `trace-log.jsonl` for pattern analysis
- [`/reference/agents/evaluator`](/reference/agents/evaluator) — the evaluator agent persona this skill orchestrates

## See Also

- Canonical source: `.claude/skills/evaluate/SKILL.md`
- Active verification patterns: `.claude/skills/evaluate/references/active-verification-patterns.md`
- Skeptic persona: `.claude/skills/evaluate/prompts/skeptic-persona.md`
- Related skills: [`mk:harness`](/reference/skills/harness), [`mk:rubric`](/reference/skills/rubric), [`mk:trace-analyze`](/reference/skills/trace-analyze)
